...
/**
 * InveDetailsE entity. @author MyEclipse Persistence Tools
 */
@Entity
@Table(name = "INVE_DETAILS_E", schema = "TRX")
@DynamicUpdate(true)
@DynamicInsert(true)
public class InveDetailsE implements java.io.Serializable {

	// Fields

	private InveDetailsEId id;
	...
	/** minimal constructor */
	public InveDetailsE(InveDetailsEId id) {
		this.id = id;
	}

	/** full constructor */
	public InveDetailsE(InveDetailsEId id, String sysOpId, Timestamp sysOpTm,
			...
			String collatNm, String collatSpec, String goodsId,
			String prcollatFact, String collatUnit, String ccy,
			BigDecimal collatRdPrice, BigDecimal collatVal, BigDecimal qty,
			Timestamp arrivalDt, Integer collatQtyBal, Integer loanQtyBal,
			Integer collatQty, Integer outQty, BigDecimal outVal,
			String collatRefNo, String cntrctNo, Integer collatOutQty,String sysOrgId) {
		super();
		this.id = id;
	...
	@EmbeddedId
	@AttributeOverrides({
			@AttributeOverride(name = "sysRefNo", column = @Column(name = "SYS_REF_NO", nullable = false, length = 50)),
			@AttributeOverride(name = "sysEventTimes", column = @Column(name = "SYS_EVENT_TIMES", nullable = false, precision = 38, scale = 0)) })
	public InveDetailsEId getId() {
		return this.id;
	}

	public void setId(InveDetailsEId id) {
		this.id = id;
	}

	@Column(name = "SYS_OP_ID", length = 50)
