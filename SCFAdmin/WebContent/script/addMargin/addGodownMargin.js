...
function pageOnResultLoad(data) {
	...
	SCFUtils.loadForm('addMargin', data);
//	SCFUtils.loadGridData('bailBillDg', SCFUtils.parseGridData(data.obj.bailBill),true);// 加载数据并保护表格。
	lookSysRelReason();
...
function pageOnPreLoad(data) {
	...
	SCFUtils.loadForm('addMargin', data);
//	SCFUtils.loadGridData('bailBillDg', SCFUtils.parseGridData(data.obj.bailBill),false);// 
//	$('#bailBillDg').datagrid('selectAll');
//	var options = $('#bailBillDg').datagrid('options');
//	options.onCheck = onCheck;
//	options.onUncheck = onUncheck;
...
function setLoanData(data) {
	...
	$('#ttlLoanBal').numberspinner('setValue', data.ttlLoanBal);
...
function onUncheck(rowIndex, rowData) {
	...
	//取消选中时，还原银承方式最低库存价值计算：最低库存价值=原最低库存价值+（本行追加保证金金额/质押率）
	//质押率
	...
	$('#regLowestValOld').val(regLowestValHD);
...
function accept() {
	...
	//银承方式最低库存价值计算：最低库存价值=原最低库存价值-（本次追加保证金金额/质押率）
	//质押率
	var pldPerc =SCFUtils.Math($('#pldPerc').numberspinner('getValue'),0.01,'mul');
	var regLowestValOld = new Number($('#regLowestValOld').val());
	var regLowestValHD = SCFUtils.Math(regLowestValOld.toFixed(2),SCFUtils.Math(ttlMarginCompen,pldPerc,'div').toFixed(2),'sub');
	$('#regLowestVal').numberspinner('setValue', regLowestValHD);
	...
	var options = {
			...
			callBackFun : function(data) {
				if (!SCFUtils.isEmpty(data.rows)) {
					var ttlCrtfAmt = 0;
					$.each(data.rows,function(i,n){
						ttlCrtfAmt = SCFUtils.Math(ttlCrtfAmt, n.crtfAmt, 'add');
					});
					$('#ttlCrtfAmt').numberspinner('setValue', ttlCrtfAmt);
				}
			}
		};
	SCFUtils.ajax(options);
...
//计算本次追加保证金金额 = 融资余额-保证金余额-融资下剩余仓单价值
function setMarginCompen(){
	var ttlLoanBal = $('#ttlLoanBal').numberspinner('getValue');
	var marginBal = $('#marginBal').numberbox('getValue');
	var ttlCrtfAmt = $('#ttlCrtfAmt').numberspinner('getValue');
	var marginCompen = SCFUtils.Math(ttlLoanBal, marginBal, 'sub');
	marginCompen = SCFUtils.Math(marginCompen, ttlCrtfAmt, 'sub');
	$('#marginCompen').numberspinner('setValue',marginCompen);
}
