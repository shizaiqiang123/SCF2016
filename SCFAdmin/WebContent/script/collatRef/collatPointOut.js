...
function pageOnFPLoad(data) {
	//退回处理时库存价值总额隐藏域的值作为初始默认值，重新计算：库存价值总额隐藏域=库存价值总额隐藏域-换入质物总价值+换出质物总价值
	//modify by YeQing 2016-11-29
	$("#ttlRegAmtHidden").val(SCFUtils.Math(SCFUtils.Math(data.obj.ttlRegAmt, data.obj.ttlInVal, 'sub'), data.obj.ttlOutVal, 'add'));
...
//	griddata = invcGd;
	...
	$.each(SCFUtils.parseGridData(inGriddata), function(i, n){
		index++;
		changeDetailsGriddata['rows'+index] = n;
	});
	$.each(SCFUtils.parseGridData(ckGriddata), function(i, n){
		index++;
		changeDetailsGriddata['rows'+index] = n;
	});
	changeDetailsGriddata._total_rows = index+1;
	...
	grid.changeDetails = SCFUtils.json2str(changeDetailsGriddata);
