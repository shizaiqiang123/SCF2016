...
function pageOnReleasePageLoad(data) {
	...
	SCFUtils.loadForm('loanForm', data);
	$("#ttlLoanBal").numberbox('setValue', SCFUtils.Math(ttlLoanBal, data.obj.payPrinAmt,'sub'));
...
function pageOnPreLoad(data) {
	...
	$('#ttlLoanBalOld').numberbox('setValue',data.obj.ttlLoanBalOld );
	SCFUtils.loadForm('loanForm', data);
	$('#ttlLoanBal').numberbox('setValue', SCFUtils.Math(data.obj.ttlLoanBalOld, data.obj.payPrinAmt, 'sub'));//还款后融资余额
	$('#OldSysRelReason').textbox('setValue',data.obj.OldSysRelReason);
...
function pageOnLoad(data) {
	...
	//getCntrctSbrSysRefNo(data.obj.cntSysRefNo);
	var options = {};
	options.data = {
		refName : 'PmtRef',
		refField : 'sysRefNo'
...
function checkPmtAmt() {
	...
	if (SCFUtils.Math(marginAmtUsed, marginBalHD, 'sub') > 0) {
		...
		//var test3 = SCFUtils.Math(ttlLoanBal, SCFUtils.Math(SCFUtils.Math(marginBal,marginAmtUsed,'sub'), pmtAmt,'add'),'sub');
		//if (test3 < 0) {
		// 原融资余额 - ( 本次还款金额+保证金余额 )<0 则提示“还款金额+保证金余额不能大于原融资余额”
		//变更：还款与保证金无关,只判断还款金额和原融资余额(JJH edit on2016-11-04)
//		if (SCFUtils.Math(ttlLoanBalOld, SCFUtils.Math(_marginBal, pmtAmt, 'add'), 'sub') < 0) {	
//			SCFUtils.alert('还款金额+保证金余额不能大于原融资余额！');
//			flag = false;
//		}
...
function checkTtlLoanBal() {
	...
	if (SCFUtils.Math(marginAmtUsed, marginBalHD, 'sub') > 0) {
		...
		//if ( test3< 0) {
		// 原融资余额 - ( 本次还款金额+保证金余额 )<0 则提示“还款金额+保证金余额不能大于原融资余额”
//		if (SCFUtils.Math(ttlLoanBalOld, SCFUtils.Math(marginBal, pmtAmt, 'add'), 'sub') < 0) {	
//			SCFUtils.alert('还款金额+保证金余额不能大于原融资余额！');
//			flag = false;
//		}
		if (SCFUtils.Math(ttlLoanBalOld, pmtAmt, 'sub') < 0) {	
...
//	$('#buyerNm').val(data.buyerNm);
	...
	// $('#loanValDt').datebox('setValue', data.loanValDt);
	// $('#loanDueDt').datebox('setValue', data.loanDueDt);
	$("#payPrinAmt").numberbox({max:parseFloat($("#ttlLoanAmt").numberbox("getValue"))});
	$('#sysRefNo').focus();
}
...
// // $('#trxDt').datebox('setValue', getDate(new Date()));
// // $('#pmtDt').datebox('setValue', getDate(new Date()));
// // if(SCFUtils.isEmpty($('#loanId').val())){
// // $('#loanId').val($('#sysRefNo').val());
// // }
...
function getSellerData() {
	...
	if('RE'===SCFUtils.FUNCTYPE || "DP" === SCFUtils.FUNCTYPE ){
		...
		obj.lmtBal = SCFUtils.Math(relQueryLmtM(lmtTp)[0].lmtBal,$('#payPrinAmt').numberbox('getValue'),'add') ;//卖方额度余额=原卖方额度余额+还款金额
		obj.lmtAllocate = SCFUtils.Math(relQueryLmtM(lmtTp)[0].lmtAllocate,$('#payPrinAmt').numberbox('getValue'),'sub') ;//本次占用额度=原来占用额度-还款金额
		obj.lmtRecover = SCFUtils.Math(relQueryLmtM(lmtTp)[0].lmtRecover,$('#payPrinAmt').numberbox('getValue'),'add') ;//归还额度=原归还额度+还款余额
...
function onNextBtnClick() {
	...
	if(checkPmtAmt()){
		...
		$.extend(mainData,grid,{
			...
			"cntrctLmtBal" : SCFUtils.Math(cntrct.lmtBal, ttlPmtAmt, 'add'),//协议_额度余额 = 原额度余额+本次释放金额
			"custLmtBal" : SCFUtils.Math(custInfo.lmtBal, ttlPmtAmt, 'add'),//客户_额度余额 = 原额度余额+本次释放金额
			"cntrctSbrLmtBal" : SCFUtils.Math(cntrctSbrInfo.buyerLmtBat, ttlPmtAmt, 'add'),//额度关联信息_额度余额 = 原额度余额+本次释放金额
			...
			'custLmtAllocate' : SCFUtils.Math(custInfo.lmtAllocate, ttlPmtAmt, 'sub'),  //客户_占用额度 = 原占用额度-本次还款金额
			'cntrctSbrLmtAllocate' : SCFUtils.Math(cntrctSbrInfo.lmtAllocate, ttlPmtAmt, 'sub'), //额度关联信息_占用额度 = 原占用额度-本次还款金额
			
			'cntrctLmtRecover' :SCFUtils.Math(cntrct.lmtRecover, ttlPmtAmt, 'add'), //协议_归还额度 =原归还额度+本次释放金额（即本次还款金额）
			'custLmtRecover' :SCFUtils.Math(custInfo.lmtRecover, ttlPmtAmt, 'add'), //客户_归还额度 =原归还额度+本次释放金额（即本次还款金额）
			'cntrctSbrLmtRecover' :SCFUtils.Math(cntrctSbrInfo.lmtRecover, ttlPmtAmt, 'add'),  //额度关联信息_归还额度 =原归还额度+本次释放金额（即本次还款金额）
	...
	var options = {
			...
			callBackFun : function(data) {
				if (data.success && !SCFUtils.isEmpty(data.rows)) {	
					...
					//$('#marginBalHD').numberbox('setValue',SCFUtils.Math(data.rows[0].marginBal, data.row[0].marginAmtUsed, 'add'));
					$('#initMarginPct').numberbox('setValue',data.rows[0].initMarginPct);
				}	
			}
		};
	SCFUtils.ajax(options);
...
if('RE'===SCFUtils.FUNCTYPE || "DP" === SCFUtils.FUNCTYPE ){
	...
	obj.lmtBal = SCFUtils.Math(relQueryLmtM(lmtTp)[0].lmtBal,$('#payPrinAmt').numberbox('getValue'),'add') ;//买方额度余额=原额度余额+本次还款金额
	obj.lmtRecover = SCFUtils.Math(relQueryLmtM(lmtTp)[0].lmtRecover,$('#payPrinAmt').numberbox('getValue'),'add') ;//归还额度=原归还额度+本次还款金额
	obj.lmtAllocate = SCFUtils.Math(relQueryLmtM(lmtTp)[0].lmtAllocate,$('#payPrinAmt').numberbox('getValue'),'sub') ;//占用额度=原占用额度-本次还款金额
}
