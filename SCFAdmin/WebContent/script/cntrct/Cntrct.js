...
function pageOnLoad(data) {
	...
	if(data.obj.busiTp == '6'){
		$("#transChgrtTwo").numberspinner('setValue',$("#transChgrt").numberspinner('getValue'));
	}
	//在新增的时候给paySts定为 0 
...
function queryCollatRegM(cntrctNo) {
	...
	if(SCFUtils.OPTSTATUS == 'EDIT' || SCFUtils.OPTSTATUS == 'VIEW'||SCFUtils.OPTSTATUS == 'DELETE')queryId = 'Q_P_000563';
	if(SCFUtils.FUNCTYPE =='RE'||SCFUtils.FUNCTYPE=='FP'||SCFUtils.FUNCTYPE=='DP')queryId = 'Q_P_000441';
	var options = {
...
function queryCntrctPat() {
	...
	if(SCFUtils.OPTSTATUS == 'EDIT' || SCFUtils.OPTSTATUS == 'VIEW'||SCFUtils.OPTSTATUS == 'DELETE')queryId = 'Q_P_000565';
	if(SCFUtils.FUNCTYPE =='RE'||SCFUtils.FUNCTYPE=='FP'||SCFUtils.FUNCTYPE=='DP')queryId ='Q_P_000439';
	var options = {
		url : SCFUtils.AJAXURL,
		data : {
			queryId : queryId,
...
function onNextBtnClick() {
	...
	if($("#busiTp").combobox('getValue')==6){
		$("#transChgrt").numberspinner('setValue',$("#transChgrtTwo").numberspinner('getValue'));
	}
	var mainData = SCFUtils.convertArray('cntrctForm');
	
	if (mainData.busiTp == '0' || mainData.busiTp == '1' || mainData.busiTp == '2' || mainData.busiTp == '6'|| mainData.busiTp == '7') {
...
//	}, 
	...
	/*
	 * var collatTp = [{"id":'0',"text":"[0] 其他"},{"id":'1',"text":"[1]
	 * 质押"},{"id":'2',"text":"[2] 抵押"},{"id":'3',"text":"[3]
	 * 保证"},{"id":'4',"text":"[4] 信用"}];
	 * $("#collatTp").combobox('loadData',collatTp); var lmt_tp =
	 * [{"id":'0',"text":"循环","selected":true},{"id":'1',"text":"单笔"}];
...
function loadChgBusiTp(custTp) {  
	...
	if(custTp == '2'){
		$.each(options.columns[0], function(i,v) {
			if(v.field == 'acctPeriod')options.columns[0][i].hidden = true;
			if(v.field == 'openactGraceDay')options.columns[0][i].hidden = true;
			if(v.field == 'graceDay')options.columns[0][i].hidden = true;
			if(v.field == 'loanPct')options.columns[0][i].hidden = true;
			if(v.field != 'loanPct'||v.field != 'acctPeriod'||v.field != 'openactGraceDay'||v.field != 'graceDay'
			&& v.width == '12.5%')options.columns[0][i].width = '20%';
		});
...
//		$("#sbrTable").datagrid("hideColumn",'graceDay');
		...
		$.each(options.columns[0], function(i,v) {
			if(v.field == 'acctPeriod')options.columns[0][i].hidden = false;
			if(v.field == 'openactGraceDay')options.columns[0][i].hidden = false;
			if(v.field == 'graceDay')options.columns[0][i].hidden = false;
			if(v.field == 'loanPct')options.columns[0][i].hidden = false;
			if(v.field != 'loanPct'||v.field != 'acctPeriod'||v.field != 'openactGraceDay'||v.field != 'graceDay'
			&&v.width == '20%')options.columns[0][i].width = '12.5%';
		});
...
//		$("#sbrTable").datagrid("showColumn",'graceDay');
	...
	$("#finaTpTd").hide();
	$('tr[id=Tr17]').hide();
	$("td[id=openLoanAmtTd]").show();
	$("td[id=openLoanAmtTlTd]").show();
	if (custTp == '0' || custTp == '1') {
		// $('#finaTp').combobox({
		// required : true
		// });
	...
	} else if (custTp == '4' || custTp == '5') {// 动产质押静态和动态
		$('#Tr1').hide();
		$('#Tr2').hide();
		$('#Tr3').hide();
		$('tr[id=Tr4]').show();
	...
	if(custTp == '6'){
		...
		$("td[id=openLoanAmtTd]").hide();
		$("td[id=openLoanAmtTlTd]").hide();
	}
