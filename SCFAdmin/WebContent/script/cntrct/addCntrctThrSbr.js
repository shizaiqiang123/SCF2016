...
function pageLoad(win) {
	...
	if (row.busiTp == '2') {
		$('tr[id=Tr3]').hide();
		$('tr[id=Tr4]').hide();
		$('#loanPct').numberspinner({
			required : false
		});
		$('#acctPeriod').numberspinner({
			required : false
		});
		$('#openactGraceDay').numberspinner({
			required : false
		});
		$('#graceDay').numberspinner({
			required : false
		});
	}
	if (row.busiTp == '3') {
	...
	if (row.op === 'add') {
		...
		SCFUtils.loadForm('addCntrctSbrForm', row);
		/*
		 * $('#sysRefNo').val(row.sysRefNo); $('#trxId').val(row.trxId);
		 * $('#cntrctNo').val(row.cntrctNo); $('#selId').val(row.selId);
		 * $('#buyerId').val(row.buyerId);
		 * $('#sellerInstCd').val(row.sellerInstCd); $('#selNm').val(row.selNm);
