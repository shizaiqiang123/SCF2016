	...
	if (!SCFUtils.isEmpty(data.obj.collateList)) {
		for (var i = 0; i < data.obj.collateList._total_rows; i++) {
			...
			$('#collatTable').datagrid('appendRow', {
				...
				subCata : data.obj.collateList[key].subCata,
				producer : data.obj.collateList[key].producer,
				goodsCharacter : data.obj.collateList[key].goodsCharacter,
				unit : data.obj.collateList[key].unit,
				ccy : data.obj.collateList[key].ccy,
				price : data.obj.collateList[key].price,
...
function pageOnLoad(data) {
	...
	if (data.obj.busiTp == '11') {
		$('#pldPerc').numberspinner('setValue', data.obj.pldPerc);
		queryThrCollatRegM(data.obj.cntrctNo);
	}
...
function pageOnReleasePageLoad(data) {
	...
	if (data.obj.busiTp == '10' || data.obj.busiTp == '11') {
		$('#pldPerc').numberspinner('setValue', data.obj.pldPerc);
...
// 查询现货静态下的质押货物
function queryThrCollatRegM(cntrctNo) {
	var options = {
		url : SCFUtils.AJAXURL,
		data : {
			queryId : 'Q_P_000498',
		...
		callBackFun : function(data) {
			if (data.success && !SCFUtils.isEmpty(data.rows)) {
				if (SCFUtils.FUNCTYPE == 'FP') {
					SCFUtils
							.loadGridData('collatTable', data.rows, false, true);
				} else {
					SCFUtils.loadGridData('collatTable', data.rows, true, true);
				}
			}
...
function getLmtData() {
	...
	$.each(datas, function(i, n) {
		...
		if ('PM' === SCFUtils.FUNCTYPE) {
			...
			SCFUtils.getRefNo(options);
			objHD.sysRefNo = $('#lmtSysRefNo').val();
		} else {
			if (relLmtERe(n.buyerId)[0] == undefined) {
				options.data = {
					refName : 'LmtRef',
					refField : 'lmtSysRefNo',
					cacheType : 'non'
				...
				objHD.sysRefNo = relLmtERe(n.buyerId)[0].sysRefNo;
			}
		}
		objHD.buyerId = n.buyerId;// 买方客户编号
		objHD.buyerNm = n.buyerNm;// 买方客户编号
		objHD.selId = n.selId;// 卖方客户编号
		objHD.selNm = n.selNm;// 卖方客户编号
		objHD.theirRef = n.sysRefNo;// 关联编号
		objHD.cntrctNo = n.cntrctNo;// 协议编号
		objHD.lmtAmt = n.buyerLmtAmt;// 额度金额
		objHD.lmtBal = n.buyerLmtBat;// 额度余额
		objHD.lmtSts = 0;// 额度状态：0.可用 1.冻结
		objHD.lmtTp = 0;// 额度类型：0.买方额度1.卖方额度
		objHD.busiTp = $("#busiTp").combobox('getValue');// 业务类别
		lmt['rows' + i] = objHD;
	});
	lmt._total_rows = datas.length + 1;
	// obj.trxDt = trxDt;//交易日期
	obj.selId = $("#selId").val();// 卖方客户编号
	obj.selNm = $("#selNm").val();// 卖方客户编号
...
function onNextBtnClick() {
	...
	if (mainData.busiTp == '0' || mainData.busiTp == '1'
			|| mainData.busiTp == '2' || mainData.busiTp == '6'
			|| mainData.busiTp == '7' || mainData.busiTp == '11') {
		var data = $('#sbrTable').datagrid('getData');
		if (data.total == 0) {
			SCFUtils.alert('请添加关联客户！');
			return;
		}
	}
	 ...
	 */
	// 货物信息
	if (mainData.busiTp == '4' || mainData.busiTp == '5'
			|| mainData.busiTp == '10' || mainData.busiTp == '11') {
		...
		for (var i = 0; i < length; i++) {
			...
			row.unit = collatList[key].unit;
			row.subCata = collatList[key].subCata;
			row.producer = collatList[key].producer;
			row.goodsCharacter = collatList[key].goodsCharacter;
			row.ccy = collatList[key].ccy;
			row.price = collatList[key].price;
...
// 修改一条数据
function editCollat() {
	var row = $('#collatTable').datagrid('getSelections');
	if (row.length == 1) {
		row = row[0];
		row.op = 'edit';
...
function editCollatSuccess(data) {
	var row = $('#collatTable').datagrid('getSelected');
	var rowIndex = $('#collatTable').datagrid('getRowIndex', row);

	var rowsData = SCFUtils.getGridData('collatTable', false);
	var rowsDatas = SCFUtils.getSelectedGridData('collatTable', false);

	var array = [];
	var gridData = SCFUtils.getGridData('collatTable', false);
	...
	var flag = true;
	$.each(rowsDatas, function(a, n) {
		for (var i = 0; i < array.length; i++) {
			if (array[i] == n.goodsId) {
	...
	$.each(rowsDatas, function(j, k) {
		if (k.goodsId == data.goodsId) {
			flag = false;
		}

	});
	if (contains(array, data.goodsId)) {
		SCFUtils.alert("该商品已经添加！");
		flag = false;
		return false;
	}
...
// 查询一条数据
function inqCollat() {
	var selectRow = $('#collatTable').datagrid('getSelected');
	var rowIndex = $('#collatTable').datagrid('getRowIndex', selectRow);
	if (selectRow) {
		var rows = $('#collatTable').datagrid('getRows');// 获取所有当前加载的数据行
...
function inqCollatSuccess(data) {
	var row = $('#collatTable').datagrid('getSelected');
	var rowIndex = $('#collatTable').datagrid('getRowIndex', row);
	$('#collatTable').datagrid('refreshRow', rowIndex);
}
...
function addSbrRow() {
	...
	var sbrData = $('#sbrTable').datagrid('getData');
	if (sbrData.total == 1) {
		SCFUtils.alert('关联客户信息只能添加一笔！');
		return;
	}
	...
	row.selId = $('#selId').val();
	row.selNm = $('#selNm').val();
	row.lmtAmt = SCFUtils.Math($('#lmtAmt').numberbox('getValue'), buyerLmtAmt,
			'sub');
...
function editSbrRow() {
	...
	if (row.length == 1) {
		...
		row.trxId = $('#sysRefNo').val();
		row.sellerInstCd = $('#sellerInstCd').val();
		row.lmtAmt = SCFUtils.Math($('#lmtAmt').numberbox('getValue'),
				buyerLmtAmt, 'sub');
		row.selNm = $('#selNm').val();
		row.busiTp = $('#busiTp').combobox('getValue');
		if ('RE' === SCFUtils.FUNCTYPE) {
...
// 修改一条数据
function inqSbrRow() {
	var selectRow = $('#sbrTable').datagrid('getSelected');
	var rowIndex = $('#sbrTable').datagrid('getRowIndex', selectRow);
	if (selectRow) {
		var rows = $('#sbrTable').datagrid('getRows');// 获取所有当前加载的数据行
		var data = rows[rowIndex];
...
function loadChgBusiTp(custTp) {
	...
	if (custTp == '2') {
		...
		/*
		 * $.each(options.columns[0], function(i, v) { if (v.field ==
		 * 'acctPeriod') options.columns[0][i].hidden = false; if (v.field ==
		 * 'openactGraceDay') options.columns[0][i].hidden = false; if (v.field ==
		 * 'graceDay') options.columns[0][i].hidden = false; if (v.field ==
		 * 'loanPct') options.columns[0][i].hidden = false; if (v.field !=
		 * 'loanPct' || v.field != 'acctPeriod' || v.field != 'openactGraceDay' ||
		 * v.field != 'graceDay' && v.width == '20%')
	...
	if (custTp == '2') {
		...
		/*
		 * $('#loanRt').numberspinner({ required : false });
		 */
		...
		if (SCFUtils.CURRENTPAGE == 0) {
			 ...
			 * $('#loanRt').numberspinner({ required : true });
	...
	SCFUtils.setNumberspinnerReadonly('initGartPct', true);
}
