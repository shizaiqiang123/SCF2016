...
function pageOnLoad(data) {
	...
	// $('#sbrInfoDiv').hide();
	/*
	 * $('#collatInfoDiv').show(); $('#partyInfoDiv').show();
	 */
}
...
function pageOnFPLoad(data) {
	...
	ajaxbrNm(data);
	loadChgBusiTp(data.obj.busiTp);
	$('#transChgrt').numberspinner('setValue', data.obj.transChgrt);
	queryGLColla(data.obj.cntrctNo);
	queryCntrctPat();// 查询关联监管方信息
	if (data.obj.busiTp == '4') {
		$('#pldPerc').numberspinner('setValue', data.obj.pldPerc);
...
function getLmtData() {
	...
	$.each(datas, function(i, n) {
		...
		if ('PM' === SCFUtils.FUNCTYPE) {
			...
			if (QueryLmtERe(n.buyerId)[0] == undefined) {
				options.data = {
					refName : 'LmtRef',
					refField : 'lmtSysRefNo',
					cacheType : 'non'
				...
				objHD.sysRefNo = QueryLmtERe(n.buyerId)[0].sysRefNo;
			}
		}
