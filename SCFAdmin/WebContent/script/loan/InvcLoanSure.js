...
//	$('#cntrctNo').val(data.obj.sysRefNo);
	...
	$('#lmtAllocate').val($('#lmtAllocateHd').val());
	/*
	 * var refRequest = {}; refRequest.data = { refName : 'LoanRef', refField :
	 * 'sysRefNo' }; SCFUtils.getRefNo(refRequest);
	 */

	querySelInfo();
	// setDueDt(data.obj.sysRefNo);
	queryInsure(data.obj.cntrctNo);
//	queryBuyerInfo(data.obj.cntrctNo, data.obj.buyerId);//查询为了获得买方额度，现都在lmt更新
	changePayIntTp();
...
function getSellerData() {
	...
	if ('RE' === SCFUtils.FUNCTYPE || "DP" === SCFUtils.FUNCTYPE) {
		if (!SCFUtils.isEmpty(relQueryLmtE(lmtTp))) {
			obj.sysRefNo = relQueryLmtE(lmtTp).sysRefNo;
			obj.lmtBal = relQueryLmtE(lmtTp).lmtBal;// 卖方额度余额=原卖方额度余额-融资金额
			obj.lmtAllocate = relQueryLmtE(lmtTp).lmtAllocate;// 本次占用额度=原来占用额度+融资金额
		}
	} else {
		...
		if (!SCFUtils.isEmpty(relQueryLmtM(lmtTp))) {
			obj.lmtBal = SCFUtils.Math(relQueryLmtM(lmtTp).lmtBal, $(
					'#ttlLoanAmt').numberbox('getValue'), 'sub');// 卖方额度余额=原卖方额度余额-融资金额
			obj.lmtAllocate = SCFUtils.Math(relQueryLmtM(lmtTp).lmtAllocate, $(
					'#ttlLoanAmt').numberbox('getValue'), 'add');// 本次占用额度=原来占用额度+融资金额
		}
...
function onNextBtnClick() {
	...
	if(mainData.isConfirm==0){
		var confirmFlag = 2;
	}else if(mainData.isConfirm == 1){
		var confirmFlag = 0;
	}
	$.extend(mainData, {
...
function loadTable() {
	...
	if (data) {
		if ('PM' === SCFUtils.FUNCTYPE) {
			...
			SCFUtils.ajax(options);
		} else if ('RE' === SCFUtils.FUNCTYPE || 'DP' === SCFUtils.FUNCTYPE ) {
			var options = {
				url : SCFUtils.AJAXURL,
				data : {
					queryId : 'Q_P_000702',
...
function appendFieldTotable(data) {
	...
	$.each(data, function(i, n) {
		...
		invLoanAvalHd = SCFUtils.Math(n.invLoanAval, n.invLoanBal, 'add');
		totalLoan = SCFUtils.Math(totalLoan, invLoanAvalHd, 'add');
	});
	$("#totalLoan").numberbox('setValue', totalLoan);
}
