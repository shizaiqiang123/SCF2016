...
function pageOnLoad(data) {
	...
	var date = new Date();
	$("#loanValDt")
			.datebox("setValue", SCFUtils.dateFormat(date, "yyyy-MM-dd"));// 20161123融资起始日默认当前时间
...
function changeLoanTp() {
	...
	if ($('#loanTp').combobox('getValue') == '4') {
		$('#BillInfoDiv').hide();
		/*
		 * $('#marginAmt').numberbox({ required : false, readonly : true });
		 */
	} else {
		$('#BillInfoDiv').show();
		/*
		 * $('#marginAmt').numberbox({ required : true, readonly : true });
		 */
	}
}
...
function loadPoMTable() {
	...
	var optionsPo = {
		...
		callBackFun : function(data) {
			if (data.success) {
				if (!SCFUtils.isEmpty(data.rows)) {
					SCFUtils.loadGridData('poTable', data.rows, true, true);// 加载的时候就保护数据
				} else {
					$("#CollaterDiv").parent("div").parent("div").attr("style",
							"display:none");
				}
			} else {
				SCFUtils.alert("查询失败");
...
function getPoSuccess(data) {
	...
	$('#ttlLoanAmt').numberbox('setValue', data.poAmt);
	$('#buyerId').focus();
	if ($('#poNo').val() != null) {
...
function getSellerData() {
	...
	if ('RE' === SCFUtils.FUNCTYPE || "DP" === SCFUtils.FUNCTYPE) {
		...
		obj.lmtBal = relQueryE.lmtBal;// 卖方额度余额=原卖方额度余额-融资金额
		obj.lmtAllocate = relQueryE.lmtAllocate;// 本次占用额度=原来占用额度+融资金额
		...
		obj.lmtBal = SCFUtils.Math(relQueryM.lmtBal, SCFUtils.Math($(
				'#ttlLoanAmt').numberspinner('getValue'), $('#marginAmt')
				.numberspinner('getValue'), 'sub'), 'sub');// 卖方额度余额=原卖方额度余额-融资金额
		obj.lmtAllocate = SCFUtils.Math(relQueryM.lmtAllocate, SCFUtils
				.Math($('#ttlLoanAmt').numberspinner('getValue'), $(
						'#marginAmt').numberspinner('getValue'), 'sub'), 'add');// 本次占用额度=原来占用额度+融资金额
...
function onNextBtnClick() {
	...
	if ('RE' != SCFUtils.FUNCTYPE && 'DP' != SCFUtils.FUNCTYPE) {
		if (SCFUtils.DateDiff(loanValDt, loanDueDt) > 0
		 ...
		 * SCFUtils.alert('初始保证金账号已存在！'); return; }
		 */
		/*
		 * if (checkBailAcNo(payBailAcno) || checkPayBailAcNo(payBailAcno)) {
		 * SCFUtils.alert('还款保证金账号已存在！'); return; }
		 */
		 ...
		 * SCFUtils.alert('间接客户额度余额不足！'); return; }
		 */
	}
	...
	$.extend(mainData, grid, {
		...
		'custLmtAllocate' : SCFUtils.Math(custInfo.lmtAllocate, lmtAllocate,
				'add'),// 客户信息_占用额度
		'cntrctSbrLmtAllocate' : SCFUtils.Math(cntrctSbrInfo.lmtAllocate,
				lmtAllocate, 'add'),// 额度关联信息_占用额度
		...
		'lmtBal' : SCFUtils.Math(cntrct.lmtBal, lmtAllocate, 'sub'),// 协议_额度余额 =
		// 原额度余额-本次占用金额
		'custLmtBal' : SCFUtils.Math(custInfo.lmtBal, lmtAllocate, 'sub'),// 客户信息_额度余额
		// =
		// 原额度余额-本次占用金额
		'cntrctSbrLmtBal' : SCFUtils.Math(cntrctSbrInfo.buyerLmtBat,
				lmtAllocate, 'sub'),// 额度关联信息_额度余额 = 原额度余额-本次占用金额

		'lmtCcy' : mainData.ccy, // 额度币别
		'regLowestVal' : SCFUtils.Math(ttlLoanAmt, margin_Amt, 'sub')
	// 最低库存价值
	});
	return mainData;
}
...
function changeTtlLoanAmt() {
	...
	var transChgrt = $('#transChgrt').val() * 0.01;// 手续费率
	var pdgAmt = SCFUtils.Math(ttlLoanAmt, transChgrt, 'mul');// 融资金额 * 手续费率

	var loanRt = $('#loanRt').val() * 0.01;// 正常利率
 ...
 * 
 * obj.lmtBal =
 * SCFUtils.Math(relQueryLmtM(lmtTp)[0].lmtBal,$('#ttlLoanAmt').numberbox('getValue'),'sub')
 * ;//买方额度余额=原买方额度余额-转让金额 obj.lmtAllocate =
 * SCFUtils.Math(relQueryLmtM(lmtTp)[0].lmtAllocate,$('#ttlLoanAmt').numberbox('getValue'),'add')
 * ;//本次占用额度=原来占用额度+转让金额 } obj.ttlAllocate =
 * $('#buyerTtlAllocate').val();//已占用额度 obj.theirRef = $('#sysRefNo').val();//
 * 
