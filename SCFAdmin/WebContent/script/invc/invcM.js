...
/*function getSellerData(){
	...
	var obj = {};
	var lmtTp = 1;
	if('RE'===SCFUtils.FUNCTYPE || "FP" === SCFUtils.FUNCTYPE ){
 ...
 * function onNextBtnClick() {
 * 
 * if('PM'===SCFUtils.FUNCTYPE){ if (checkDataGrid()) { return; } } var data =
 * SCFUtils.convertArray('invcMForm'); var grid = {};
 * 
 * var griddata = SCFUtils.getSelectedGridData("invcMTable",false); var custData
 * =[]; //客户表更新(可用额度，已用额度) var index = 0; var isCollect =
 * $('#isCollect').combobox("getValue"); var sysData =
 * $('#sysData').datebox('getValue'); $.each(griddata, function(key, n) { if(key !=
 * '_total_rows' ){ if("Y" == isCollect){ n.feeDt = sysData; } n.type = "ADD";
 * n.index = index; getProcPag(n); index++; var invCust = {}; //发票中的客户信息
 * invCust.sysRefNo = n.buyerId; invCust.buyerNm = n.buyerNm; invCust.invBal =
 * n.invBal; custData.push(invCust); } });
 * 
 * var cust = {}; //合并客户信息 for(var i=0;i<custData.length;i++){ var obj =
 * queryBuyerLmtAmt(custData[i].sysRefNo);//查询买方额度信息 custData[i].lmtBal =
 * obj.lmtBal; custData[i].lmtAllocate = obj.lmtAllocate; custData[i].limiting =
 * obj.limiting //买方限制条款 if(custData.length>1){ for(var
 * j=custData.length-1;j>i;j--){ if(custData[j].sysRefNo ==
 * custData[i].sysRefNo){ custData[i].invBal =
 * SCFUtils.Math(custData[i].invBal,custData[j].invBal,'add');
 * custData.splice(j, 1); } } custData[i].lmtBal =
 * SCFUtils.Math(custData[i].lmtBal,custData[i].invBal,'sub');
 * custData[i].lmtAllocate =
 * SCFUtils.Math(custData[i].lmtAllocate,custData[i].invBal,'add'); }else{
 * custData[i].lmtBal =
 * SCFUtils.Math(custData[i].lmtBal,custData[i].invBal,'sub'); //可用额度
 * custData[i].lmtAllocate =
 * SCFUtils.Math(custData[i].lmtAllocate,custData[i].invBal,'add');//已用额度 } }
 * //额度关联关系表与客户表更新 var cntrct = {}; var cntChange = {}; cust._total_rows =
 * custData.length; cntrct._total_rows = custData.length; cntChange._total_rows =
 * custData.length; var cntInfo = ""; var sbrInfo = ""; var sysRefNo =
 * $("#sysRefNo").val(); var cntrctNo = $("#cntrctNo").val(); var trxCcy =
 * $('#ccy').combobox('getValue'); var clType = 'O'; var tdType = 'T'; var
 * trxDate = $('#trxDt').datebox('getValue'); var limiting ="";
 * $.each(custData,function(i,obj){ cust['rows'+i+''] = obj; var cntrctSbrM =
 * queryCntrctSbrM(cntrctNo,obj.sysRefNo);//客户额度编号，买方Id查询关联表中的买方关联额度 var
 * buyerLmtBat = SCFUtils.Math(cntrctSbrM.buyerLmtBat,obj.invBal,'sub');
 * //关联买方额度余额-净额 cntrct['rows'+i+''] =
 * {'buyerLmtBat':buyerLmtBat,'sysRefNo':cntrctSbrM.sysRefNo}; if(buyerLmtBat<0){
 * sbrInfo += "间接客户：【"+obj.buyerNm+"】 "; } //额度变更表插入记录 cntChange['rows'+i+''] =
 * {"sysRefNo":sysRefNo+obj.sysRefNo,"cntrctNo":cntrctNo,"clType":clType,"tdType":tdType,
 * "trxDate":trxDate,"trxAmt":obj.invBal,"refNo":sysRefNo,'trxCcy':trxCcy,'custNo':obj.sysRefNo,'custNm':obj.buyerNm};
 * if(!SCFUtils.isEmpty(obj.limiting))//输出买方限制条款 limiting +=
 * "【"+obj.buyerNm+"】:"+obj.limiting+";<br>"; });
 * 
 * //griddata = SCFUtils.getGridData('invcMTable', false); grid.invc =
 * SCFUtils.json2str(griddata); grid.cust = SCFUtils.json2str(cust); grid.cntrct =
 * SCFUtils.json2str(cntrct); grid.cntChange = SCFUtils.json2str(cntChange);
 * grid.insure = SCFUtils.json2str(insure); $.extend(data, grid); if(
 * !SCFUtils.isEmpty(limiting) && ('PM'===SCFUtils.FUNCTYPE ||
 * 'FP'===SCFUtils.FUNCTYPE)){ if(sbrInfo !=""){
 * SCFUtils.alert(limiting,function(){ //买方限制条款
 * $.messager.confirm('Confirm',sbrInfo+"关联额度余额不足！",function(r){ if(r) return
 * data; // SCFUtils.SubmitAjax(data); else return data; //
 * SCFUtils.SubmitAjax(data); }); }); }else{ SCFUtils.alert(limiting,function(){
 * return data; // SCFUtils.SubmitAjax(data); }); } }else{ return data; //
