...
/*
 * getBuyerLmt function 
 * 根据data从custM表中抓取lmtBal值赋给sbr表的额度字段
 */
...
function queryCollatCompNm(data) {//测试过后删除
//	var options = {
//		url : SCFUtils.AJAXURL,
//		data : {
//			queryId : 'Q_P_000428',
...
//			var intAmtNew = n.ttlLoanBalHd*(pmtDt-n.loanValDt+1)*n.loanRt/360;
			...
			var intAmtNew = SCFUtils.Math(SCFUtils.Math(n.ttlLoanBalHd,days,'mul'),loanRtDay,'mul');
...
function getSellerHKH(selId) {//查询扣款账号信息
	var acOwnerid = selId;
	var options = {
		url : SCFUtils.AJAXURL,
		data : {
			queryId : 'Q_S_SELACNO_001',//共用的Query查询
			acOwnerid : acOwnerid
		},
		callBackFun : function(data) {
			if (data.success) {
				...
				// 卖方还款账号
				if (data.rows.length > 0) {
					$("#acNo").combobox('loadData', data.rows);//给combobox控件加载下拉数据
				}
	...
	var options = {
		...
		callBackFun : function(data) {
			if (data.success) {
				...
				// 卖方还款账号
				if (data.rows.length > 0) {
					$("#acBkNm").val(data.rows[0].acBkNm);//给combobox控件加载下拉数据
					obj = data.rows[0];
				}
...
function getMainAmt(data){//计算主页的信息
	...
	$.each(data,function(i,n){//循环invcMTable中更新的记录
		...
		ttlStruLoanPercAmt = SCFUtils.Math(SCFUtils.Math(n.struLoanAmt,perc,'mul'),ttlStruLoanPercAmt,'add');//得到每笔发票冲销融资的金额
...
function queryCntrctInfoFP(cntrctNo,sysLockBy){
	var options = {//595先同RE时一样查询cntrctE表中的值给页面显示。
			url : SCFUtils.AJAXURL,
			data : {
				queryId : 'Q_P_000595',
				cntrctNo : cntrctNo,
		...
		SCFUtils.ajax(options);
		var optionsHd = {//595查询cntrctM表中的值给Hd临时字段，为了输入ttlPmtAmt重新计算。
				url : SCFUtils.AJAXURL,
				data : {
					queryId : 'Q_P_000596',
					cntrctNo : cntrctNo,
...
function getIntData(){
	...
	if('PM' === SCFUtils.FUNCTYPE){
		...
		var obj = {};
			...
			obj.selAcNo = changeAcNo().acNo;
			obj.selAcNm = changeAcNo().acNm;
			obj.selAcBkNm = changeAcNo().acBkNm;
