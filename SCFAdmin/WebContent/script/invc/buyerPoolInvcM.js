...
function pageOnReleasePageLoad(data) {
	...
	queryPoolLineFP();
	loadTableRe();
	var payIntTp = queryCntrct(data.obj.cntrctNo).payIntTp;
	var arAvalLoanHD = queryCntrct(data.obj.cntrctNo).arAvalLoan;//从cntrct_M表拿的。arAvalLoanHD为原来的值
	$('#arAvalLoanHD').val(arAvalLoanHD);
	//复核进来arAvalLoan的初值也要是cntrctM中的arAvalLoan值，与PM保持一致
	//$('#arAvalLoan').numberbox('setValue', 0);
	$('#arAvalLoan').numberbox('setValue', arAvalLoanHD);
...
function getArAvalLoanRe() {
	...
	var arAvalLoan = $("#arAvalLoan").numberbox("getValue");//FP和Re时候arAvalLoan为cntrctM中的值。这边就要累加
