...
if('RE'===SCFUtils.FUNCTYPE || "DP" === SCFUtils.FUNCTYPE ){
	...
	obj.lmtBal = SCFUtils.Math(relQueryLmtM(lmtTp,buyerId)[0].lmtBal,$('#bailPayAmt').numberbox('getValue'),'add') ;//买方额度余额=原额度余额+本次追加保证金金额
	obj.lmtRecover = SCFUtils.Math(relQueryLmtM(lmtTp,buyerId)[0].lmtRecover,$('#bailPayAmt').numberbox('getValue'),'add') ;//归还额度=原归还额度+本次追加保证金金额
	obj.lmtAllocate = SCFUtils.Math(relQueryLmtM(lmtTp,buyerId)[0].lmtAllocate,$('#bailPayAmt').numberbox('getValue'),'sub') ;//占用额度=原占用额度-本次追加保证金金额
