...
//		SCFUtils.ajax(opt);
		...
		/*
		 * var options = { url : SCFUtils.AJAXURL, data : { queryId :
		 * 'Q_P_000018', sysRefNo : sysRefNo, sysEventTimes :sysEventTimes, },
		 * callBackFun : function(data) {
		 * if(data.success&&!SCFUtils.isEmpty(data.rows)){ rows[i].invBal =
		 * data.rows[0].invBal; rows[i].invLoanBal = data.rows[0].invLoanBal;
		 * rows[i].invLoanAval = data.rows[0].invLoanAval; rows[i].invBalHd =
		 * SCFUtils.Math(data.rows[0].invBal, data.rows[0].struLoanAmt, 'add'); //
		 * rows[i].invLoanBalHd = data.rows[0].invLoanBal; rows[i].invLoanBalHd =
		 * SCFUtils.Math(data.rows[0].invLoanBal,
		 * SCFUtils.Math(data.rows[0].struLoanAmt,data.rows[0].loanPerc/100.0000,'mul'),
		 * 'add') // rows[i].invLoanAvalHd = data.rows[0].invLoanAval;
		 * rows[i].invLoanAvalHd = SCFUtils.Math(data.rows[0].invLoanBal,
		 * SCFUtils.Math(data.rows[0].invBal,data.rows[0].loanPerc/100.0000,'mul'),
		 * 'add'); rows[i].loanTranAmt = data.rows[0].loanTranAmt;
		 * rows[i].loanBillAmt = data.rows[0].loanBillAmt; rows[i].payAmt =
		 * data.rows[0].payAmt; rows[i].loanPerc = data.rows[0].loanPerc;
		 * rows[i].struLoanAmt = data.rows[0].struLoanAmt; rows[i].invNo =
		 * data.rows[0].invNo; rows[i].invCcy = data.rows[0].invCcy;
		 * rows[i].tempTranAmt = data.rows[0].tempTranAmt; rows[i].tempBillAmt =
		 * data.rows[0].tempBillAmt; rows[i].ck=true; } } };
		 * SCFUtils.ajax(options);
		 */
...
function accept() {
	...
	$.each(data, function(i, n) {
		...
		// if(SCFUtils.Math(n.struLoanAmt, n.loanAmt, 'sub') >0){
		// var index = $("#invcMTable").datagrid('getAllRowIndex',n);
		// SCFUtils.alert('冲销金额不得大于该笔融资金额！');
		// onUncheck(index, n);
		// }
		// totalPayTranAMt= SCFUtils.Math(totalPayTranAMt, n.tempTranAmt,
		// 'add');
		// totalPayBillAmt= SCFUtils.Math(totalPayBillAmt, n.tempBillAmt,
		// 'add');
