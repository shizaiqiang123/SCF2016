...
function loadSearchBox(n,length,span){
	...
	if (n.datatype == 1) {// 文本框(修改百联)
		$('<input class="inputM1 combo" type="text" style="border:0px solid #fff;width:0px;'+height+'"/>')
...
function createDivForSearch(data){
	var div = $('<div style="display:none;float:left;width:1025px;margin-left:-32px;" id="moreSearchDiv"></div>').appendTo($('#screenDiv'));
	...
	for(var i = index;i<data.obj.search.length;i++){
		...
		$('<label class="dsB fL tR labelStyle"></label>').html(n.title+':').appendTo(li);
		var span = $('<span class="dsB fL inputWidth140" ></span>').appendTo(li);
