...
//		$('#blockAreaFooter').hide();
		...
		$("#positionDiv").removeClass("marginBot22");
		var options = {
			pageInfo : {
				functionId : "F0000000",
				name : "主页",
...
//						p = '_entry=1&'+p;
	...
	SCFUtils.entry = function(menuid, path, reqType,reqData) {
		...
		if(url.indexOf("index.jsp") > 0 ){
			...
			$("#centerDiv",window.parent.document).addClass("centerDivPadding");
			$("#centerDiv",window.parent.document).removeClass("mainPageCenterDivHight");
			$("#positionDiv",window.parent.document).addClass("marginBot22");
		}
		var options = {
				url : SCFUtils.PAGEURL,
				data : {
...
//							p = '_entry=1&'+p;
		 ...
		 * ---  以下4行代码在再次进入时将左侧导航放回  -----*/
		$("#positionDiv").removeClass("marginBot22");//正常进交易会进入上面的工作流的样式修改
		$("#sideNav").removeClass('closed');
//		$("#leftMenu").animate({'margin-left': '0px'});//animate为动画效果。左侧导航逐渐变为距离左边0px
...
//		$('#menuBody').show();
		...
		$('#sideNav').show();
		$('#bodyHr').attr("style","box-shadow: 1px 1px 5px #909090;");
		$('#centerDiv').attr("style","margin-left:19px;width:100%;margin-top:20px;min-height: 1200px;float:right;");
		$('#blockAreaFooter').attr("display","block");//进入交易页面显示按钮
		$("#bodyBox").attr("style","width:70%");
		$("#centerDiv").addClass("centerDivPadding");
		$("#centerDiv").removeClass("mainPageCenterDivHight");
...
//		});
	...
	/**
	 * 获取到原流水号，并在原流水号前缀后面拼接格式为20161124的日期格式
	 * @param index  前缀长度
	 * 
	 * */
	SCFUtils.setRefNo = function(sysRefNo,index){
		var date =SCFUtils.getcurrentdate();
		date = SCFUtils.dateFormat(date,'yyyyMMdd');
		var sysRefBefore = sysRefNo.substring(0,index);
		var sysRefAfter = sysRefNo.substring(index,sysRefNo.length);
