...
jQuery.extend({ 
   ...
   toUrlStr: function(object) { 
     ...
     for (var property in object) { 
       ...
       if (value !== undefined) {
    	   var n = jQuery.toJSONString(property);
    	   n = n.replace(new RegExp('"','gm'),'');
    	   value = value.replace(new RegExp('"','gm'),'');
    	   results.push(n + '=' + encodeURIComponent(value));
       }
     } 
     return '&' + results.join('&') + ''; 
   } 
...
jQuery.extend({ 
	   toJSONStr: function(object) { 
	     ...
	     case 'object': 
	       ...
	       var results = []; 
	       for (var property in object) { 
	         var value = jQuery.toJSONStr(object[property]); 
	     ...
	     case 'array': 
	       var results = []; 
	       for (var i = 0; i < object.length; i++) { 
	         var value = jQuery.toJSONStr(object[i]); 
	         if (value !== undefined) results.push(value); 
	       } 
	       return '[' + results.join(',') + ']'; 
	       break; 
