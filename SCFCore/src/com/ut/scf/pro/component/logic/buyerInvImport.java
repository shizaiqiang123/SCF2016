...
@Component("buyerInvImport")
public class buyerInvImport implements ITemplateReformat {
	
	@Resource(name="refNoManager")  
	IReferenceNo refNoManager;	
	protected IDaoHelper daoHelper;
	private static Logger logger = LoggerFactory.getLogger(buyerInvImport.class);
	@Override
	public void reformat(FuncDataObj logicObj)
		throws Exception {
		logger.info("==================buyerInvImport.reformat  start==========================");			
		...
		logger.info("==================buyerInvImport.reformat  end==========================");		
	}
	...
	private void checkData(FuncDataObj logicObj,List list) throws Exception {
		...
		try {
			...
			String trxSelName = trxData.getString("buyerNm");//买方名称
			...
			if(!trxSelName.equals(buyerName)){
				sb.append("上传的核心企业名称：【"+buyerName+"】,与协议中的核心企业名称：【"+trxSelName+"】不一致。\r\n");
			}
