...
@Component("invTemplateReformatImpl")
public class InvTemplateReformatImpl implements ITemplateReformat {
	
	@Resource(name="refNoManager")  
	IReferenceNo refNoManager;	
	protected IDaoHelper daoHelper;
	private static Logger logger = LoggerFactory.getLogger(InvTemplateReformatImpl.class);
	@Override
	...
	private void checkData(FuncDataObj logicObj,List list) throws Exception {
		...
		try {
			...
			String trxSelName = trxData.getString("selNm");//卖方名称
	...
	private List getSysRefNo(FuncDataObj logicObj,List list) throws Exception {
		...
		for (int i = 0; i < list.size(); i++) {			
			...
			map.put("acctPeriod", acctPeriod);
			map.put("invBal",invAmt-acctAmt );
