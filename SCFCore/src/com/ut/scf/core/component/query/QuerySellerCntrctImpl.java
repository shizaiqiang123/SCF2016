 ...
 *
 */
@Component("querySellerCntrct")
public class QuerySellerCntrctImpl implements IQueryComponent{
	...
	public FuncDataObj queryData(FuncDataObj logicObj) {
		...
		/*String selId = trxInfo.getString("selId");
		node.setParams(selId);*/
		String sql = "select e from CntrctE e where e.id.sysRefNo=? and e.theirRef is not null and e.sysTrxSts = 'M'";
		...
		if(processResult.hasRecord()){
			...
			if(!obj.isEmpty()){
				...
				for (Map mod:obj) {
					...
					second.setId(mod.get("sysRefNo").toString()+mod.get("cntrctNo").toString());
					second.setParentId(first.getId());
					second.setText(mod.get("sysRefNo").toString());
