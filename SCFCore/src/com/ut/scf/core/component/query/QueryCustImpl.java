 ...
 */
@Component("queryCust")
public class QueryCustImpl implements IQueryComponent{
	...
	public FuncDataObj queryData(FuncDataObj logicObj) {
		...
		String sql = "select e from CntrctSbrE e where e.buyerId=? and e.trxId is not null and sysTrxSts = 'M'";
		...
		if(processResult.hasRecord()){
			...
			if(!obj.isEmpty()){
				...
				for (Map mod:obj) {
					...
					first.setId(mod.get("buyerId").toString()+mod.get("buyerNm").toString());
					first.setText(mod.get("buyerNm").toString());
					first.setIsparent("Y");
					menu.addItem(first);
					
					MenuItem second  = new MenuItem();
					second.setId(mod.get("sysRefNo").toString()+mod.get("selId").toString());
					second.setParentId(first.getId());
					second.setText(mod.get("selNm").toString());
					second.setIsparent("Y");
					menu.addItem(second);
					
					MenuItem third  = new MenuItem();
					third.setId(mod.get("sysRefNo").toString()+mod.get("trxId").toString());
					third.setParentId(second.getId());
					third.setText(mod.get("trxId").toString());
					third.setIsparent("N");
					menu.addItem(third);
				}
