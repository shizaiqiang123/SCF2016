 ...
 * @since 2016-9-8
 */
@Component("batchPrePayLoanDueAlarmProcessor")
@Scope(ConfigurableBeanFactory.SCOPE_SINGLETON)
public class BatchPrePayLoanDueAlarmProcessor extends AbsRunableTask {
	...
	public void execute() {
		...
		if(!msgMap4Add.keySet().isEmpty() || !msgMap4Update.keySet().isEmpty()) {
			generator.generatePointPrePayLoanDueAlarm(msgMap4Add,msgMap4Update);
		}
	...
	private void getMsgMapAtALL(){
		...
		String Sql="SELECT SYS_OP_ID,SYS_REL_ID,SYS_AUTH_ID,CNTRCT_NO,SEL_ID,SEL_NM,CCY,BUYER_ID,SYS_REF_NO,LOAN_DUE_DT,TTL_LOAN_BAL,BUSI_TP,TRUNC(LOAN_DUE_DT)-TRUNC(SYSDATE) AS DAYOFF FROM TRX.LOAN_M  WHERE TTL_LOAN_BAL > 0 AND BUSI_TP = 2 AND SYS_LOCK_FLAG = 'F' AND SYS_TRX_STS = 'M' AND (TRUNC(LOAN_DUE_DT) - TRUNC(SYSDATE)) BETWEEN 1 AND " + dayOffConfig + "  ORDER BY SYS_REF_NO ASC";
		...
		if(null != back && back.size() > 0) {
			...
			String busiTp = null;
			String key = null;
			System.out.println("-------------BatchPrePayLoanDueAlarmProcessor.getMsgMapAtALL.back.size()---" + back.size());
			...
			while(in.hasNext()) {
				...
				if(null != inner.get("SYS_OP_ID") && !((String)inner.get("SYS_OP_ID")).trim().equals("")) {
					...
					mapMsg.put("msgTitle", "预付类融资[" + (String)inner.get("SYS_REF_NO") + "]即将到期");
					
					yBuff = new StringBuffer("");
					yBuff.append("预付类-融资编号[" + (String)inner.get("SYS_REF_NO") + "],");
	...
	private void getAdviceLinkMap(){
		...
		xBuff.append(" FROM STD.ADVICE_LINK WHERE MESSAGE_TYPE = " + AdviceConstants.MESSAGE_TYPE_PREPAYLOANDUEALARM);
