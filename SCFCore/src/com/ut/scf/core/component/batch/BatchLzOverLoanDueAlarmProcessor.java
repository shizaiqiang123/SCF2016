 ...
 * @since  2016-09-06
 */
@Component("batchLzOverLoanDueAlarmProcessor")
@Scope(ConfigurableBeanFactory.SCOPE_SINGLETON)
public class BatchLzOverLoanDueAlarmProcessor extends AbsRunableTask {
	...
	private void getMsgMapAtALL(){
		...
		String cols = "SYS_OP_ID,SYS_REL_ID,SYS_AUTH_ID,CNTRCT_NO,SEL_ID,BUYER_ID,SYS_REF_NO,LOAN_DUE_DT,TTL_LOAN_BAL,TTL_LOAN_AMT,BUSI_TP,CCY,SEL_NM,DAYOFF"; 
		
		StringBuffer xBuff = new StringBuffer("");
		xBuff.append(" SELECT ");
		xBuff.append(" SYS_OP_ID,SYS_REL_ID,SYS_AUTH_ID, CNTRCT_NO,SEL_ID,BUYER_ID,SYS_REF_NO,LOAN_DUE_DT,TTL_LOAN_BAL,TTL_LOAN_AMT,BUSI_TP,CCY,SEL_NM,TRUNC(SYSDATE)-TRUNC(LOAN_DUE_DT) AS DAYOFF ");
		xBuff.append(" FROM TRX.LOAN_M ");                                //0，应该进入逾期状态
		xBuff.append(" WHERE SYS_TRX_STS = 'M' AND SYS_LOCK_FLAG = 'F' AND TTL_LOAN_BAL > 0 AND (TRUNC(LOAN_DUE_DT) < TRUNC(SYSDATE)) ");
		...
		if(null != back && back.size() > 0) {
			...
			String key = null;
			System.out.println("-------------BatchLzOverDueLoanAlarmProcessor.getMsgMapAtALL.back.size()---" + back.size());
			...
			while(in.hasNext()) {
				...
				if(null != inner.get("SYS_OP_ID") && !((String)inner.get("SYS_OP_ID")).trim().equals("")) {
					...
					mapMsg.put("msgTitle", "融资编号[" + (String)inner.get("SYS_REF_NO") + "]已逾期");
					
					yBuff = new StringBuffer("");
					yBuff.append("融资编号[" + (String)inner.get("SYS_REF_NO")     + "],");
					yBuff.append("授信客户名称[" + (String)inner.get("SEL_NM") + "],");
					yBuff.append("融资到期日为[" + dateStr + "],已逾期,");
//					yBuff.append("融资金额[" + (java.math.BigDecimal)inner.get("TTL_LOAN_AMT")     + "],");
...
//					yBuff.append("逾期天数[" + (java.math.BigDecimal)inner.get("DAYOFF")     + "].");
				...
				if(null != inner.get("SYS_REL_ID") && !((String)inner.get("SYS_REL_ID")).trim().equals("")) {
					...
					mapMsg2.put("msgTitle", "融资编号[" + (String)inner.get("SYS_REF_NO") + "]已逾期");
...
//					yBuff.append("逾期天数[" + (java.math.BigDecimal)inner.get("DAYOFF")     + "].");
					...
					mapMsg2.put("LOAN_NO", loan_no);
					mapMsg2.put("busiTp", busi_tp);
					key = cntrct_no + ";" + sel_id + ";" + loan_no + ";" + rec_id;
					
					if(null != adviceLinkMap.get(key)) {
						mapMsg2.put("MSG_ID", (String) adviceLinkMap.get(key));
						msgMap4Update.put(((String)inner.get("SYS_REL_ID")).trim() + "-" + (String)inner.get("SYS_REF_NO"), mapMsg2);
					} else {
						   msgMap4Add.put(((String)inner.get("SYS_REL_ID")).trim() + "-" + (String)inner.get("SYS_REF_NO"), mapMsg2);
					}
				...
				if(null != inner.get("SYS_AUTH_ID") && !((String)inner.get("SYS_AUTH_ID")).trim().equals("")) {
					...
					mapMsg3.put("msgTitle", "融资[" + (String)inner.get("SYS_REF_NO") + "]逾期预警");
					
					yBuff = new StringBuffer("");
					yBuff.append("融资[" + (String)inner.get("SYS_REF_NO")     + "]逾期预警,");
					yBuff.append("逾期日期[" + dateStr + "],");
					yBuff.append("融资余额[" + (java.math.BigDecimal)inner.get("TTL_LOAN_BAL")    + "],");
					yBuff.append("逾期天数[" + (java.math.BigDecimal)inner.get("DAYOFF")     + "].");
					
					mapMsg3.put("recId",    ((String)inner.get("SYS_REL_ID")).trim());
	...
	private void getAdviceLinkMap(){
		...
		xBuff.append(" FROM STD.ADVICE_LINK WHERE MESSAGE_TYPE = " + AdviceConstants.MESSAGE_TYPE_OVERDUELOANALARM);
