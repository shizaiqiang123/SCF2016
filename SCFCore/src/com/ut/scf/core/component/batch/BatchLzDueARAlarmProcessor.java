 ...
 * @since  2015-12-24
 */
@Component("batchLzDueARAlarmProcessor")
@Scope(ConfigurableBeanFactory.SCOPE_SINGLETON)
public class BatchLzDueARAlarmProcessor extends AbsRunableTask {
	...
	private void getMsgMapAtALL(){
		...
		xBuff.append(" SELECT ");
		xBuff.append(" SYS_OP_ID,SYS_REL_ID,CNTRCT_NO,SEL_ID,BUYER_ID,INV_NO,INV_DUE_DT,INV_BAL, TRUNC(INV_DUE_DT)-TRUNC(SYSDATE) AS DAYOFF ");
		xBuff.append(" FROM TRX.INVC_M ");                                                //0，应该进入逾期状态
		xBuff.append(" WHERE INV_BAL > 0 AND ((TRUNC(INV_DUE_DT) - TRUNC(SYSDATE)) BETWEEN 1 AND " + dayOffConfig + ") ");
		...
		if(null != back && back.size() > 0) {
			...
			while(in.hasNext()) {
				...
				if(null != inner.get("SYS_OP_ID") && !((String)inner.get("SYS_OP_ID")).trim().equals("")) {
					...
					mapMsg.put("msgTitle", "应收账款[" + (String)inner.get("INV_NO") + "]将到期预警");
					//mapMsg.put("msgText",  "应收账款[" + (String)inner.get("INV_NO") + "]将到期预警,到期日期[" + dateStr + "]将到期预警,到期日期["  );
					
					yBuff = new StringBuffer("");
					yBuff.append("应收账款[" + (String)inner.get("INV_NO")     + "]将到期预警,");
				...
				if(null != inner.get("SYS_REL_ID") && !((String)inner.get("SYS_REL_ID")).trim().equals("")) {
					...
					mapMsg2.put("msgTitle", "应收账款[" + (String)inner.get("INV_NO") + "]将到期预警");
