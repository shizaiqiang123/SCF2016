 ...
 * @since  2015-12-24
 */
@Component("batchLzOverDueLoanAlarmProcessor")
@Scope(ConfigurableBeanFactory.SCOPE_SINGLETON)
public class BatchLzOverDueLoanAlarmProcessor extends AbsRunableTask {
	...
	private void getMsgMapAtALL(){
		...
		String cols = "SYS_OP_ID,SYS_REL_ID,SYS_AUTH_ID,CNTRCT_NO,SEL_ID,BUYER_ID,SYS_REF_NO,LOAN_OVERDUE_DT,TTL_LOAN_BAL,DAYOFF"; 
		
		StringBuffer xBuff = new StringBuffer("");
		xBuff.append(" SELECT ");
		xBuff.append(" SYS_OP_ID,SYS_REL_ID,SYS_AUTH_ID, CNTRCT_NO,SEL_ID,BUYER_ID,SYS_REF_NO,LOAN_OVERDUE_DT,TTL_LOAN_BAL, TRUNC(SYSDATE)-TRUNC(LOAN_OVERDUE_DT) AS DAYOFF ");
		xBuff.append(" FROM TRX.LOAN_M ");                                //0，应该进入逾期状态
		xBuff.append(" WHERE TTL_LOAN_BAL > 0 AND (TRUNC(LOAN_OVERDUE_DT) <= TRUNC(SYSDATE)) ");
		...
		if(null != back && back.size() > 0) {
			...
			while(in.hasNext()) {
				...
				String dateStr = null;
				try {
					TIMESTAMP rebackinner = (TIMESTAMP)inner.get("LOAN_OVERDUE_DT");  
				...
				if(null != inner.get("SYS_OP_ID") && !((String)inner.get("SYS_OP_ID")).trim().equals("")) {
					...
					mapMsg.put("msgTitle", "融资[" + (String)inner.get("SYS_REF_NO") + "]逾期预警");
				...
				if(null != inner.get("SYS_REL_ID") && !((String)inner.get("SYS_REL_ID")).trim().equals("")) {
					...
					mapMsg2.put("msgTitle", "融资[" + (String)inner.get("SYS_REF_NO") + "]逾期预警");
