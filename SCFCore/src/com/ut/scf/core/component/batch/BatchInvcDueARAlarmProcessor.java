 ...
 * @since  2016-09-06
 */
@Component("batchInvcDueARAlarmProcessor")
@Scope(ConfigurableBeanFactory.SCOPE_SINGLETON)
public class BatchInvcDueARAlarmProcessor extends AbsRunableTask {
	...
	public void execute() {
		...
		if(!msgMap4Add.keySet().isEmpty() || !msgMap4Update.keySet().isEmpty()) {
			generator.generatePoint2PointMsg4DueARAlarm(msgMap4Add,msgMap4Update);
		}
		
	}
	
	private int dayOffConfig = 0;//dayOffConfig天数在task中的para节点中配置。initDayOffConfig方法中设置
	...
	private void getMsgMapAtALL(){
		...
		String cols = "SYS_OP_ID,SYS_REL_ID,CNTRCT_NO,SEL_ID,SEL_NM,BUYER_ID,BUYER_NM,INV_NO,INV_DUE_DT,INV_AMT,INV_BAL,BUSI_TP,INV_CCY,STRU_LOAN_AMT,DAYOFF"; 
		//只查询转让完发票状态为1 的发票
		StringBuffer xBuff = new StringBuffer("");
		xBuff.append(" SELECT ");
		xBuff.append(" SYS_OP_ID,SYS_REL_ID,CNTRCT_NO,SEL_ID,SEL_NM,BUYER_ID,BUYER_NM,INV_NO,INV_DUE_DT,INV_AMT,INV_BAL,BUSI_TP,INV_CCY,STRU_LOAN_AMT, TRUNC(INV_DUE_DT)-TRUNC(SYSDATE) AS DAYOFF ");
		xBuff.append(" FROM TRX.INVC_M ");                                                //0，应该进入逾期状态
		xBuff.append(" WHERE SYS_TRX_STS = 'M' AND SYS_LOCK_FLAG = 'F' AND INV_STS = 1 AND INV_BAL > 0 AND ((TRUNC(INV_DUE_DT) - TRUNC(SYSDATE)) BETWEEN 0 AND " + dayOffConfig + ") ");//现今距离到期日1-5天的invc
		xBuff.append(" ORDER BY INV_NO ASC ");
		
		List<Map<String, Object>> back = daoHelper.executeOrentSql(xBuff.toString(), cols.split(","));
		
		if(null != back && back.size() > 0) {//查询到有发票即将到期
			...
			String inv_no      = null;
			String rec_id      = null;
			String busi_tp      = null;//为了显示新增存入到预警三张表
			
			String key = null;
			
			System.out.println("-------------BatchLzDueARAlarmProcessor.getMsgMapAtALL.back.size()---" + back.size());
			...
			BigDecimal struLoanAmt = new BigDecimal(0);
			Iterator<Map<String, Object>> in = back.iterator();//迭代遍历back
			while(in.hasNext()) {
				...
				String dateStr = null;
				try {
					TIMESTAMP rebackinner = (TIMESTAMP)inner.get("INV_DUE_DT");  //得到发票到期日，并将日期格式转化为string
				...
				if(inner.get("STRU_LOAN_AMT") != null){
					struLoanAmt = (BigDecimal)inner.get("STRU_LOAN_AMT");
				}
				
				String id = UUIdGenerator.getUUId();//得到随机的id
				
				if(null != inner.get("SYS_OP_ID") && !((String)inner.get("SYS_OP_ID")).trim().equals("")) {//如果申请者id不为空
					mapMsg = new HashMap<String, String>();
					mapMsg.put("msgTextId", id);//将随机取的id给通知内容id
					mapMsg.put("msgSendTp", String.valueOf(AdviceConstants.ADV_MSG_SEND_TP_MSG));//通知发送类型
					...
					mapMsg.put("msgTitle", "应收账款编号[" + (String)inner.get("INV_NO") + "]即将到期");
					//mapMsg.put("msgText",  "应收账款[" + (String)inner.get("INV_NO") + "]将到期预警,到期日期[" + dateStr + "]将到期预警,到期日期["  );
					
					yBuff = new StringBuffer("");
					yBuff.append("应收账款编号[" + (String)inner.get("INV_NO")     + "],");
					yBuff.append("授信客户名称[" + (String)inner.get("SEL_NM") + "],");
					yBuff.append("间接客户名称[" + (String)inner.get("BUYER_NM") + "],");
					yBuff.append("应收账款将于[" + dateStr + "]到期,");
					yBuff.append("应收账款净额[" + (java.math.BigDecimal)inner.get("INV_BAL")    + "],");
					yBuff.append("已冲销金额[" + struLoanAmt    + "],");
					yBuff.append("币种[" + (String)inner.get("INV_CCY")     + "].");
...
//					mapMsg.put("busiTp", busi_tp);
				...
				if(null != inner.get("SYS_REL_ID") && !((String)inner.get("SYS_REL_ID")).trim().equals("")) {//如果复核者id不为空
					
//					mapMsg2 = new HashMap<String, String>();
//					mapMsg2.put("msgTextId", id);//将随机取的id给通知内容id
//					mapMsg2.put("msgSendTp", String.valueOf(AdviceConstants.ADV_MSG_SEND_TP_MSG));//通知发送类型
...
//					
//					mapMsg2.put("msgTitle", "应收账款编号[" + (String)inner.get("INV_NO") + "]即将到期");
//					//mapMsg.put("msgText",  "应收账款[" + (String)inner.get("INV_NO") + "]将到期预警,到期日期[" + dateStr + "]将到期预警,到期日期["  );
//					
//					yBuff = new StringBuffer("");
//					yBuff.append("应收账款编号[" + (String)inner.get("INV_NO")     + "],");
...
//					yBuff.append("间接客户名称[" + (String)inner.get("BUYER_NM") + "],");
//					yBuff.append("应收账款将于[" + dateStr + "]到期,");
//					yBuff.append("应收账款金额[" + (java.math.BigDecimal)inner.get("INV_AMT")    + "],");
...
//					busi_tp = (String)inner.get("BUSI_TP");
//					
//					mapMsg2.put("CNTRCT_NO", cntrct_no);
//					mapMsg2.put("SEL_ID", sel_id);
//					mapMsg2.put("BUYER_ID", buyer_id);
//					mapMsg2.put("INV_NO", inv_no);
//					mapMsg2.put("busiTp", busi_tp);
//					
//					key = cntrct_no + ";" + sel_id + ";" + buyer_id + ";" + inv_no + ";" + rec_id;
//					if(null != adviceLinkMap.get(key)) {
//						mapMsg2.put("MSG_ID", (String) adviceLinkMap.get(key));
//						msgMap4Update.put(((String)inner.get("SYS_REL_ID")).trim() + "-" + (String)inner.get("INV_NO"), mapMsg2);
//					} else {
//						   msgMap4Add.put(((String)inner.get("SYS_REL_ID")).trim() + "-" + (String)inner.get("INV_NO"), mapMsg2);
//					}
				}
	...
	private void getAdviceLinkMap(){
		...
		xBuff.append(" FROM STD.ADVICE_LINK WHERE MESSAGE_TYPE = " + AdviceConstants.MESSAGE_TYPE_DUEARALARM);
