 ...
 *
 */
@Component("batchCustImpProcessor")
@Scope("prototype")
public class BatchCustImpProcessor  extends AbsRunableTask{
	...
	public void reformat(FuncDataObj logicObj) throws Exception {		
			APLogUtil
					.getUserLogger()
					.info("==================batchCustImpProcessor  start==========================");
			...
			for (int i = 0; i < list.size(); i++) {
				...
				dataObj.mergeResponse(saveCustImpInfo(logicObj.getReqData(), map)); //统一事务
				dataObj.mergeResponse(saveCustBuyerInfo(logicObj.getReqData(), map)); //统一事务
			}						
			daoHelper.execUpdate(dataObj);
			APLogUtil.getUserLogger().info("==================batchCustImpProcessor  end==========================");		
	}
	...
	private List checkData(FuncDataObj logicObj, List list) throws Exception {		
		...
		List<String> licenceNoList = new ArrayList<String>();
		try {
			for (int i = 0; i < list.size(); i++) {
				Map map = (Map) list.get(i);
				Object licenceNo = map.get("licenceNo");
				if(licenceNo!=null &&StringUtils.isNotEmpty(licenceNo.toString().trim())){
					if(!licenceNoList.contains((String)licenceNo)){
						reqList.add(map);
						licenceNoList.add((String)licenceNo);
					}
				}
			}
			return reqList;
	...
	private List getSysRefNo(FuncDataObj logicObj, List list) throws Exception {
		...
		refPara.setRefname("custImpRefNo");
	 ...
	 * @throws Exception
	 */
	private FuncDataObj saveCustBuyerInfo(JSONObject trxDom, Map map) throws Exception{
		APLogUtil.getUserLogger().info("==============batchCustImpProcessor.saveCustBuyerInfo  s==============");	
		FuncDataObj dataObj = new FuncDataObj();
		...
		List<Map> obj = chickCustBuyer(trxDom,map);
		LogicNode mainLogic = new LogicNode();
		mainLogic.setTablename("TRX.CUST_BUYER");
		mainLogic.setType("C");	
		...
		APLogUtil.getUserLogger().info("==============batchCustImpProcessor.saveCustBuyerInfo  e==============");
		return dataObj;
	}
	 ...
	 * @throws Exception
	 */
	public FuncDataObj saveCustImpInfo(JSONObject trxDom, Map map)
			throws Exception {
		APLogUtil.getUserLogger().info("==============batchCustImpProcessor.saveCustImpInfo  s==============");	
		FuncDataObj dataObj = new FuncDataObj();
		JSONObject trxData = JsonHelper.getTrxObject(trxDom);
		if (trxData.containsKey("sysBusiUnit")) {
			String sysBusiUnit = trxData.getString("sysBusiUnit");
		...
		List<Map> obj = checkLicenceNo(trxDom,map);
		LogicNode mainLogic = new LogicNode();
		mainLogic.setTablename("TRX.CUST_IMP");
		mainLogic.setType("C");
		mainLogic.setCascadeevent("N");
		dataObj.setReqParaObj(mainLogic);
		if (!obj.isEmpty()) {//如果数据库中已经有这个营业执照的供应商，则只更新CUST_IMP表。
			trxData.putAll(map);
			trxData.put("sysRefNo", obj.get(0).get("sysRefNo"));
			dataObj.setReqData(trxData);
			ILogicComponent t = ClassLoadHelper.getBusiComponetProcessor("trxEditRecord");
		...
		APLogUtil.getUserLogger().info("==============batchCustImpProcessor.saveCustImpInfo  e==============");	
		return dataObj;
	}

	private List<Map> checkLicenceNo(JSONObject trxDom,Map map) {		
		FuncDataObj dataObj = new FuncDataObj();
		JSONObject trxData = JsonHelper.getTrxObject(trxDom);
		if (trxData.containsKey("sysBusiUnit")) {
			String sysBusiUnit = trxData.getString("sysBusiUnit");
		...
		queryNode.setSql("select c from TRX.CustImp c where  c.licenceNo=?");
	...
	private List<Map> chickCustBuyer(JSONObject trxDom,Map map){
		FuncDataObj dataObj = new FuncDataObj();
		JSONObject trxData = JsonHelper.getTrxObject(trxDom);
		if (trxData.containsKey("sysBusiUnit")) {
			String sysBusiUnit = trxData.getString("sysBusiUnit");
		...
		String licenceNo = map.get("licenceNo").toString();
		QueryNode queryNode = new QueryNode();
		queryNode.setSql("select c from TRX.CustBuyer c where c.buyerId=? and c.licenceNo=?");
		queryNode.setType(LogicNode.LOGIC_TYPE_SQL);
		String paras = new StringBuffer()
		.append(buyerId).append(",").append(licenceNo).toString();
