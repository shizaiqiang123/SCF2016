 ...
 * @author JinJh
 * @since  2016-09-06
 * query 应收账款到期日<系统当前日期
 */
@Component("batchInvcOverDueARAlarmProcessor")
@Scope(ConfigurableBeanFactory.SCOPE_SINGLETON)
public class BatchInvcOverDueARAlarmProcessor extends AbsRunableTask {
	...
	private void initDayOffConfig() {
		...
		String paraDt = (String) trxDataPara.getProterties().get("para"); // 发票到期前设置天数(应收账款到期日＞系统当前日期不需要设置逾期天数)
	...
	private void getMsgMapAtALL(){
		...
		xBuff.append(" SELECT ");
		xBuff.append(" SYS_OP_ID,SYS_REL_ID,CNTRCT_NO,SEL_ID,SEL_NM,BUYER_ID,BUYER_NM,INV_NO,INV_DUE_DT,INV_AMT,INV_BAL,BUSI_TP,INV_CCY,STRU_LOAN_AMT, TRUNC(SYSDATE)-TRUNC(INV_DUE_DT) AS DAYOFF ");
		xBuff.append(" FROM TRX.INVC_M ");
		xBuff.append(" WHERE SYS_TRX_STS = 'M' AND SYS_LOCK_FLAG = 'F' AND INV_STS = 1 AND INV_BAL > 0 AND (TRUNC(INV_DUE_DT) < TRUNC(SYSDATE)) ");
		...
		if(null != back && back.size() > 0) {
			...
			while(in.hasNext()) {
				...
				String dateStr = null;
				try {
					TIMESTAMP rebackinner = (TIMESTAMP)inner.get("INV_DUE_DT");  
				...
				if(null != inner.get("SYS_OP_ID") && !((String)inner.get("SYS_OP_ID")).trim().equals("")) {
					...
					mapMsg.put("msgTitle", "应收账款编号[" + (String)inner.get("INV_NO") + "]已逾期");
					
					yBuff = new StringBuffer("");//动态字符串添加逾期的具体信息，赋值给通知的text
					...
					yBuff.append("应收账款到期日为[" + dateStr + "],已逾期,");
...
//					yBuff.append("逾期天数[" + (java.math.BigDecimal)inner.get("DAYOFF")     + "].");
					...
					mapMsg.put("msgText", yBuff.toString());//给通知的text文本框赋值
					mapMsg.put("busiTp", (String)inner.get("BUSI_TP"));//给通知的text文本框赋值
				...
				if(null != inner.get("SYS_REL_ID") && !((String)inner.get("SYS_REL_ID")).trim().equals("")) {
					...
					mapMsg2.put("msgTitle", "应收账款编号[" + (String)inner.get("INV_NO") + "]已逾期");
...
//					yBuff.append("逾期天数[" + (java.math.BigDecimal)inner.get("DAYOFF")     + "].");
					...
					mapMsg2.put("msgText", yBuff.toString());//给通知的text文本框赋值
					mapMsg2.put("busiTp", (String)inner.get("BUSI_TP"));//给通知的text文本框赋值
