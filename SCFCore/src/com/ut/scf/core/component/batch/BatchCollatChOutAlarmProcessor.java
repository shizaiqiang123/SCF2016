...
/**
 * 存货类-协议过期预警：相关的协议下质物还有库存，提醒质物出库
 * @author YeQing
 * @since  2016-9-8
 */
@Component("batchCollatChOutAlarmProcessor")
@Scope(ConfigurableBeanFactory.SCOPE_SINGLETON)
public class BatchCollatChOutAlarmProcessor extends AbsRunableTask {
	...
	public void execute() {
		...
		if(!msgMap4Add.keySet().isEmpty() || !msgMap4Update.keySet().isEmpty()) {
			generator.generatePointCollatchOutAlarm(msgMap4Add,msgMap4Update);
		}
	...
	private void getMsgMapAtALL(){
		String cols = "SYS_REF_NO,SYS_OP_ID,SYS_REL_ID,SYS_AUTH_ID,SEL_ID,SEL_NM,LMT_DUE_DT,BUSI_TP,DAYOFF"; 
		
		String Sql = "SELECT T1.SYS_REF_NO,T2.SYS_OP_ID, T2.SYS_REL_ID,T2.SYS_AUTH_ID,T1.SEL_ID,T1.SEL_NM, T1.LMT_DUE_DT, T1.BUSI_TP,TRUNC(T1.LMT_DUE_DT)-TRUNC(SYSDATE) AS DAYOFF FROM TRX.CNTRCT_M T1 INNER JOIN  TRX.COLLAT_CH_M T2 ON T1.SYS_REF_NO  = T2.CNTRCT_NO WHERE (TRUNC(T1.LMT_DUE_DT) - TRUNC(SYSDATE)) BETWEEN 1 AND " + dayOffConfig + " AND T2.COLLAT_OUT_QTY > 0 GROUP BY T1.SYS_REF_NO,T2.SYS_OP_ID,T2.SYS_REL_ID, T2.SYS_AUTH_ID,T1.SEL_ID,T1.SEL_NM, T1.LMT_DUE_DT,T1.BUSI_TP";
		...
		if(null != back && back.size() > 0) {
			...
			String busiTp = null;
			String key = null;
			System.out.println("-------------BatchCollatChOutAlarmProcessor.getMsgMapAtALL.back.size()---" + back.size());
			...
			while(in.hasNext()) {
				...
				if(null != inner.get("SYS_OP_ID") && !((String)inner.get("SYS_OP_ID")).trim().equals("")) {
					...
					mapMsg.put("msgTitle", "存货类协议[" + (String)inner.get("SYS_REF_NO") + "]即将到期");
					
					yBuff = new StringBuffer("");
					yBuff.append("存货类-协议编号[" + (String)inner.get("SYS_REF_NO") + "],");
	...
	private void getAdviceLinkMap(){
		...
		xBuff.append(" FROM STD.ADVICE_LINK WHERE MESSAGE_TYPE = " + AdviceConstants.MESSAGE_TYPE_COLLATCHOUTALARM);
