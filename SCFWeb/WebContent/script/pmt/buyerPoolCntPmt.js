...
/*
 * 查询买方保理里的关联客户 从协议关联表里去查询selId和selNm new on 2016-10-17 by JJH
 */
...
/*
 * function showLookUpWindow() { var sysRefNo = $('#cntrctNo').val(); var
 * options = { title : '合同号信息查询', reqid : 'I_P_000120', data : { 'cntrctNo' :
 * sysRefNo }, onSuccess : cntrctNoSuccess }; SCFUtils.getData(options);
 *  }
 */
		...
		callBackFun : function(data) {
			if (data.success && !SCFUtils.isEmpty(data.rows)) {
				...
				$('#lmtAllocate').numberbox('setValue',
						data.rows[0].lmtAllocate);
				$('#lmtAllocateHd').numberbox('setValue',
						data.rows[0].lmtAllocate);
			}
		}
	};
...
/*
 * function cntrctNoSuccess(data) { $("#loanId").textbox('setValue',
 * data.sysRefNo); }
 */
...
function onNextBtnClick() {
	...
	$.extend(mainData, {
		...
		"cntrctLmtBal" : SCFUtils.Math(cntrct.lmtBal, ttlLoanBal, 'add'),// 协议_额度余额
																			// =
																			// 原额度余额+核销应收账款总额
		"custLmtBal" : SCFUtils.Math(custInfo.lmtBal, ttlLoanBal, 'add'),// 客户_额度余额
																			// =
																			// 原额度余额+核销应收账款总额
		"cntrctSbrLmtBal" : SCFUtils.Math(cntrctSbrInfo.buyerLmtBat,
				ttlLoanBal, 'add'),// 额度关联信息_额度余额 = 原额度余额+核销应收账款总额
		...
		'cntrctLmtAllocate' : SCFUtils.Math(cntrct.lmtAllocate, ttlLoanBal,
				'sub'), // 协议_占用额度 = 原占用额度-核销应收账款总额
		'custLmtAllocate' : SCFUtils.Math(custInfo.lmtAllocate, ttlLoanBal,
				'sub'), // 客户_占用额度 = 原占用额度-核销应收账款总额
		'cntrctSbrLmtAllocate' : SCFUtils.Math(cntrctSbrInfo.lmtAllocate,
				ttlLoanBal, 'sub'), // 额度关联信息_占用额度 = 原占用额度-核销应收账款总额

		'cntrctLmtRecover' : SCFUtils
				.Math(cntrct.lmtRecover, ttlLoanBal, 'add'), // 协议_归还额度
																// =原归还额度+核销应收账款总额
		'custLmtRecover' : SCFUtils
				.Math(custInfo.lmtRecover, ttlLoanBal, 'add'), // 客户_归还额度
																// =原归还额度+核销应收账款总额
		'cntrctSbrLmtRecover' : SCFUtils.Math(cntrctSbrInfo.lmtRecover,
				ttlLoanBal, 'add'), // 额度关联信息_归还额度 =原归还额度+核销应收账款总额
...
function onCheck(rowIndex, rowData) {
	/*
	 * rowData.ck = true; // $('#accetpFlag').val("false"); // 是否已接受编辑。
	 * 
	 * var index = $("#invcMTable").datagrid('getAllRowIndex', rowData);
	 * list.push({ index : rowIndex, tempTranAmt : rowData.tempTranAmt,
	 * tempBillAmt : rowData.tempBillAmt, struLoanAmt : rowData.struLoanAmt });
	 * $('#invcMTable').datagrid('updateRow', { index : rowIndex, row :
	 * {//选中时直接冲销最大值 // 当选中一行数据时，给字段重新改值 struLoanAmt :rowData.invBal } });
	 * onClickRow(rowIndex);
	 */
}
...
function onUncheck(rowIndex, rowData) {
	/*
	 * rowData.ck = false; $('#accetpFlag').val("false");
	 * $('#invcMTable').datagrid('updateRow', { index : rowIndex, row : {
	 * invLoanBal : rowData.invLoanBalHd, invLoanAval : rowData.invLoanAvalHd,
	 * invBal : rowData.invBalHd, struLoanAmt : 0.00, loanAmt :
	 * rowData.loanAmtHd, } }); endEditing();
...
function loadTable() {
	...
	if (SCFUtils.CURRENTPAGE == '1'
			&& ('PM' == SCFUtils.FUNCTYPE || 'FP' == SCFUtils.FUNCTYPE)) {// 原来为('FP'
																			// ==
																			// SCFUtils.FUNCTYPE
																			// &&
																			// FPLoadTable
		...
		if ($("#acNo").combobox('getValue') == null
				|| $("#acNo").combobox('getValue') == "") {
	...
	if ('PM' === SCFUtils.FUNCTYPE) {
		var optionsLoan = {
			...
			callBackFun : function(data) {
				if (data.success) {
					if (!SCFUtils.isEmpty(data.rows)) {
						SCFUtils.loadGridData('loanMTable', data.rows, true,
								true);// 加载的时候就保护数据，struLoanAmt自动计算
						// forEach(data.rows);
						$.each(data.rows, function(i, n) {
							$('#loanMTable').datagrid('updateRow', {
								index : i,
								row : {
									loanId : n.sysRefNo,// 将流水号给loanId
									ttlLoanBalHd : n.ttlLoanBal
								// LOAN_M. Ttl_Loan_Bal = Ttl_Loan_Bal-
								// Pay_Prin_Amt.
								}
							});
						});
					} else {
						$("#loanMDiv").parent("div").parent("div").attr(
								"style", "display:none");
					}
				} else {
					SCFUtils.alert("查询失败");
...
function loadLoanMTableRe() {
	...
	var optionsLoan = {
		...
		callBackFun : function(data) {
			if (data.success) {
				if (!SCFUtils.isEmpty(data.rows)) {
					SCFUtils.loadGridData('loanMTable', data.rows, true, true);// 加载的时候就保护数据，struLoanAmt自动计算
					// forEach(data.rows);
							...
							$('#loanMTable').datagrid(
									'updateRow',
									{
										...
										row : {
											...
											ttlLoanBalHd : SCFUtils.Math(
													n.ttlLoanBal, n.payPrinAmt,
													'add')
										// LOAN_M. Ttl_Loan_Bal = Ttl_Loan_Bal-
										// Pay_Prin_Amt.
										}
					...
					$("#loanMDiv").parent("div").parent("div").attr("style",
							"display:none");
				}
			} else {
				SCFUtils.alert("查询失败");
 ...
 * 融资信息的字段计算，在加载时候就计算
 * 
 * @data 参数data传来的值为Q_P_000659查询到的数据 data.rows
 */
function calculateLoanM(data) {
	...
	$.each(data, function(i, n) {
		if (n.payIntTp == 1) {// 1=先收息
			// 计算intAmt，规则：payIntTp = ‘先收息’ intAmt=loanM.intAmt;payIntTp=‘利随本清’
			// INT_AMT= Ttl_Loan_Bal*【（还款日-起息日）+1】* LOAN_RT/360
			...
			$('#loanMTable').datagrid(
					'updateRow',
					{
						...
						row : {
							...
							pmtAmt : pmtAmtNew,
							ttlLoanBal : SCFUtils.Math(n.ttlLoanBalHd,
									payPrinAmtNew, 'sub')
						// LOAN_M. Ttl_Loan_Bal = Ttl_Loan_Bal- Pay_Prin_Amt.
						}
					});
		}
		if (n.payIntTp == 2) {
			...
			// var intAmtNew =
			// n.ttlLoanBalHd*(pmtDt-n.loanValDt+1)*n.loanRt/360;
			var days = SCFUtils.DateDiff(SCFUtils.dateFormat(pmtDt,
					'yyyy-MM-dd'), n.loanValDt);
			var loanRtRe = SCFUtils.Math(n.loanRt, 0.01, 'mul');
			var loanRtDay = SCFUtils.Math(loanRtRe, 360, 'div');
			var intAmtNew = SCFUtils.Math(
					SCFUtils.Math(n.ttlLoanBalHd, days, 'mul'), loanRtDay,
					'mul').toFixed(2);
			if (SCFUtils.Math(ttlPmtAmt, SCFUtils.Math(n.ttlLoanBalHd,
					n.intAmt, 'add'), 'sub') > 0) {// 大于，本次还款本金=融资余额，本次还款利息=利息总额
				var payPrinAmtNew = n.ttlLoanBalHd;
				var payIntAmtNew = intAmtNew;
				ttlPmtAmt = SCFUtils.Math(ttlPmtAmt, SCFUtils.Math(
						payPrinAmtNew, payIntAmtNew, 'add'), 'sub'); // 计算的到剩余可归还融资敞口金额
			} else {
				if (SCFUtils.Math(ttlPmtAmt, intAmtNew, 'sub') > 0) {// 剩余可归还融资敞口金额>
																		// INT_AMT
					var payPrinAmtNew = SCFUtils.Math(ttlPmtAmt, intAmtNew,
							'sub');
					var payIntAmtNew = intAmtNew;
				} else {
					var payPrinAmtNew = 0;
			...
			callBackFun : function(data) {
				...
				$('#invcMTable').datagrid(
						'updateRow',
						{
							index : i,
							row : {
								invBalHd : SCFUtils.Math(n.invBal,
										n.struLoanAmt, 'add'),// 不能丢要赋值，用于onchangStruLoanAmt
								invPmtRef : invPmtRef,
								invRef : n.sysRefNo.substring(0, 8),// n.sysRefNo
																	// =
																	// invcxxxpmtxxxx
								sysRefNo : n.sysRefNo,
								lastPayDt : SCFUtils.getcurrentdate(),
								invLoanRefNo : loanId
										+ n.sysRefNo.substring(0, 8),
								loanAmt : temp,
								loanAmtHd : temp,
							}
						});
...
function changeStruLoanAmt() {
	...
	for (var i = 0; i < data.length; i++) {
		if (SCFUtils.Math(ttlPmtAmt, data[i].invBalHd, 'sub') >= 0) {// 判断剩余冲销金额是否大于该笔应收账款净额(用临时)，大于：该笔struLoanAmt
																		// =
																		// invBal，小于struLaonAmt=剩余PmtAmt
...
function getMainAmt(data) {// 计算主页的信息
	// 更新可融资余额 CNTRCT_M. AR_AVAL_LOAN = AR_AVAL_LOAN –SUM(小页（STRU_LOAN_AMT*
	// LOAN_PERC）)+ SUM(Pay_Prin_Amt);
	...
	$.each(data, function(i, n) {// 循环invcMTable中更新的记录
		...
		ttlStruLoanPercAmt = SCFUtils.Math(SCFUtils.Math(n.struLoanAmt, perc,
				'mul'), ttlStruLoanPercAmt, 'add');// 得到每笔发票冲销融资的金额=sum(STRU_LOAN_AMT*
													// LOAN_PERC)
		// ttlPayPrinAmt =
		// SCFUtils.Math(ttlPayPrinAmt,n.payPrinAmt,'add');//SUM(Pay_Prin_Amt);
	});
	$.each(loanData, function(i, n) {// 循环invcMTable中更新的记录
	// ttlStruLoanPercAmt =
	// SCFUtils.Math(SCFUtils.Math(n.struLoanAmt,n.loanPerc,'mul'),ttlStruLoanPercAmt,'add');//得到每笔发票冲销融资的金额
	...
	arAvalLoan = SCFUtils.Math(SCFUtils.Math(arAvalLoan, ttlStruLoanPercAmt,
			'sub'), ttlPayPrinAmt, 'add');
	if (arAvalLoan < 0)
		arAvalLoan = 0;
	...
	// 更新主页的poolline：CNTRCT_M.pool_line=pool_line- SUM(小页（STRU_LOAN_AMT*
	// LOAN_PERC）)=poolLine-ttlStruLoanPercAmt
...
function restorelmtAmt() {
	...
	var lmtAmt = SCFUtils.Math($("#lmtAmtHd").numberbox('getValue'),
			ttlPayPrinAmt, 'add');// 还原卖方额度，lmtAmt+sum本次还款金额
...
/*
 * 更新买方还款时候的买方、卖方额度时候需要抓取的三表M表数据 查询custM sbrM cntrctM 表
 */
// 查询协议表
...
/*
 * 打包利息信息入Int表 公式：CURR_PAY_INT= Ttl_Loan_Bal*【（还款日-起息日）+1】* LOAN_RT/360
...
function updateLmt(){
	...
	if(buyerobj.length > 0){
		buyerLmtAmt = SCFUtils.Math(buyerobj.lmtBal,$("#ttlPmtAmt").numberbox("getValue"),'add');
	...
	if(selerobj.length > 0){
		if(selerobj.lmtAmt > 0){
			selerLmtAmt = SCFUtils.Math(selerobj.lmtBal,$("#ttlPmtAmt").numberbox("getValue"),'add');
		}
