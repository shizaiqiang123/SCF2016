...
//					$('#selAcBkNm').val(data.rows[0].acBkNm);
					$('#selAcNoBL').val(data.rows[0].acNo);
				} else {
...
//计算最大还款金额
function setPayMaxAmt() {
	var ttlLoanBal = $("#ttlLoanBal").numberbox('getValue');// 借款余额
	var currFinCost = $('#currFinCost').numberbox('getValue');// 应收费用
	var intAmt = $('#intAmt').val();// 利息
	$("#payPrinAmt").numberbox('setValue', ttlLoanBal);// 本次还本金金额
	var payMaxAmt = SCFUtils.Math(
			SCFUtils.Math(ttlLoanBal, currFinCost, 'add'), intAmt, 'add');
	payMaxAmt = parseFloat(payMaxAmt).toFixed(2);
	$('#payMaxAmt').numberbox('setValue', payMaxAmt);// 最大还款金额
}
...
function changeTtlPmtAmt() {
	...
	var currFinCost = $('#currFinCost').numberbox('getValue');// 应收费用
	var intAmt = $('#intAmt').val();// 利息
	if (SCFUtils.isEmpty(ttlPmtAmt)) {
		return;
	}
	var payPrinAmt = SCFUtils.Math(SCFUtils.Math(ttlPmtAmt, currFinCost, 'sub'),
			intAmt, 'sub');
	payPrinAmt = parseFloat(payPrinAmt).toFixed(2);
	$('#payPrinAmt').numberbox('setValue', payPrinAmt);// 本次还本金金额
}

function checkPayPrinAmt() {
	var flag = false;
	var ttlPmtAmt = $("#ttlPmtAmt").numberbox('getValue');// 本次还款金额
	var payMaxAmt = $("#payMaxAmt").numberbox('getValue');// 本次还最大金额
	
	var currFinCost = $("#currFinCost").numberbox('getValue');//应收费用
	var payIntAmt = $("#payIntAmt").numberbox('getValue');// 本次还利息金额
	
	var payMinAmt = SCFUtils.Math(currFinCost, payMinAmt, 'add');
	if (SCFUtils.Math(ttlPmtAmt, payIntAmt, 'sub')< 0) {
		SCFUtils.alert("本次还款金额不能小于利息费用总和！");
		flag = true;
	}else if (SCFUtils.Math(ttlPmtAmt, payMaxAmt, 'sub') > 0) {
		SCFUtils.alert("本次还款金额不能超过最大还款金额！");
		flag = true;
	} 
	return flag;
}
