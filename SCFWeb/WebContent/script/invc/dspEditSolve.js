...
function pageLoad(win) {
	...
	$('#fixRsn').val(row.fixRsn);
	if (row.op === 'add') {
		$('#acctPeriod').val(row.acctPeriod);
		$('#invCcy').combobox('setValue',row.invCcy);
	}else if(row.op === 'edit'){
		SCFUtils.setFormReadonly('#invcMChgForm',true);
		SCFUtils.setNumberboxReadonly('dspAmt', false);//修改时，争议金额可以编辑
		SCFUtils.setComboReadonly('fixRsn', false);//修改时，解决原因可以编辑
