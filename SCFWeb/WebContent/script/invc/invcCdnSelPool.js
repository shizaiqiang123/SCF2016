...
function ajaxBox() {
	var data = [ {// 业务类别（0有追，1无追，2 先票款后货,3 保险项下,4 现货动态 ,5 现货静态 ,6 池融资）
		"id" : '0',
		"text" : "国内有追索权保理"
	}, {
		"id" : '1',
...
function onNextBtnClick() {
	...
	var custLmtAllocate = 0;
	var cntrctSbrLmtAllocate = 0;

	if (SCFUtils.Math(custInfo.lmtAllocate, cnAmt, 'sub') > 0) {
		custLmtAllocate = SCFUtils.Math(custInfo.lmtAllocate, cnAmt, 'sub');
	} else {
		custLmtAllocate = 0;
	}

	if (SCFUtils.Math(cntrctSbrInfo.lmtAllocate, cnAmt, 'sub') > 0) {
		cntrctSbrLmtAllocate = SCFUtils.Math(cntrctSbrInfo.lmtAllocate, cnAmt,
				'sub');
	} else {
		cntrctSbrLmtAllocate = 0;
	...
	$.extend(data, {
		...
		"custLmtBal" : SCFUtils.Math(custInfo.lmtBal, cnAmt, 'add'),// 客户_额度余额 =  原额度余额+贷项清单总额
		...
		'custLmtAllocate' : custLmtAllocate, // 客户_占用额度 = 原占用额度-贷项清单总额
		'cntrctSbrLmtAllocate' : cntrctSbrLmtAllocate, // 额度关联信息_占用额度 = 原占用额度-贷项清单总额

		'custLmtRecover' : SCFUtils.Math(custInfo.lmtRecover, cnAmt, 'add'), // 客户_归还额度 =原归还额度+贷项清单总额
		'cntrctSbrLmtRecover' : SCFUtils.Math(cntrctSbrInfo.lmtRecover, cnAmt,
				'add'),	// 额度关联信息_归还额度 =原归还额度+贷项清单总额
		'cntrctArBal' : SCFUtils.Math(cntrct.arBal,cnAmt, 'sub'),//更新协议表中应收账款余额
		'cntrctOpenLoanAmt' : SCFUtils.Math(cntrct.openLoanAmt,cnAmt, 'add')//更新协议表中已用敞口
	});
	var grid = {};
	var invCbk = {};
	var fpData = [];
...
function editSuccess(data) {
	...
	if (inv_No != data.invNo && row.length>1) {//如果选中的行数>1则累加
		var cnAmt = $("#cnAmt").numberbox('getValue');
...
// 验证发票是否退回处理
function checkInvCbkPool(invNo) {
	var flag = false;
	var options = {
		url : SCFUtils.AJAXURL,
		data : data = {
