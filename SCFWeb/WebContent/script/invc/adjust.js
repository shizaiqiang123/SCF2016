...
function changeOthers(){
	var adjTp = $('#adjTp').combobox('getValue');
	if(adjTp == 1){
		SCFUtils.setFormReadonly('#cntrctDiv', false);
		SCFUtils.setDateboxReadonly('trxDt',true);
		$('#trxDt').datebox('setValue', getDate(new Date()));
		SCFUtils.setNumberspinnerReadonly('invAmt',true);
		SCFUtils.setNumberspinnerReadonly('invBal',true);
		$('#invSts').val('ADJS');
		$('#invNo').val($('#sysNo').val());
		$('#invBal').numberspinner('setValue',$('#numBal').val());
	}else if(adjTp == 0){
		var date = getDate(new Date());
		var invDueDt = $('#invDueDt').datebox('getValue');
		if(comTime(invDueDt,date)){
			SCFUtils.alert("应收账款到期后不能再撤销!");
			$('#adjTp').combobox('setValue',1);
			return;
		}
		$('#invSts').val('CAL');
		$('#invNo').val($('#sysNo').val()+'CAL');
		$('#invBal').numberspinner('setValue',0);
		
		SCFUtils.setFormReadonly('#cntrctDiv', true);
		SCFUtils.setComboReadonly('adjTp',false); 
	}
...
function pageOnLoad(data) {
	...
	$('#numBal').val($('#invBal').numberspinner('getValue'));
	$('#sysNo').val($('#sysRefNo').val());
	$('#trxDt').datebox('setValue', getDate(new Date()));
	$('#adjTp').combobox('setValue','1');
}
