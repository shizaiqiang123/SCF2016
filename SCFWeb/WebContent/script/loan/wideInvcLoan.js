...
function getSellerData() {
	...
	if('RE'===SCFUtils.FUNCTYPE || "DP" === SCFUtils.FUNCTYPE ){
		...
		if (!SCFUtils.isEmpty(relQueryLmtM(lmtTp))) {
			obj.lmtBal = SCFUtils.Math(relQueryLmtM(lmtTp).lmtBal,$('#ttlLoanAmt').numberbox('getValue'),'sub') ;//卖方额度余额=原卖方额度余额-融资金额
			obj.lmtAllocate = SCFUtils.Math(relQueryLmtM(lmtTp).lmtAllocate,$('#ttlLoanAmt').numberbox('getValue'),'add') ;//本次占用额度=原来占用额度+融资金额
		}
