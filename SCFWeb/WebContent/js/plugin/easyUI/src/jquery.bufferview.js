...
//		var rows = state.data.rows.slice(this.renderedCount, this.renderedCount+opts.pageSize);
		...
		var tmprow = [];
		if (!SCFUtils.isEmpty(state.data) && !SCFUtils.isEmpty(state.data.rows)){
			tmprow = state.data.rows;
		}
		var rows = tmprow.slice(index, index+opts.pageSize);
		if (rows.length){
			opts.onFetch.call(target, page, rows);
			callback.call(this, rows);
		} else {
			var param = $.extend({}, opts.baseParams,opts.queryParams, {
