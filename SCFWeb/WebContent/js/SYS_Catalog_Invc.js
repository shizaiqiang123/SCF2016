...
//						alert(JSON.stringify(data));
		...
		var select = $(
				'<input class="combo easyui-combobox" style="border:1px solid #fff;width:'+length+'px;">').attr('name', n.field).attr('id',
...
function formaterAmt(data){
/*	if(''!=data.invMinAmt){
		data.invMinAmt = data.invMinAmt.replace(/\,/g,"");
	}*/
	if(''!=data.invMaxAmt){
		data.invMaxAmt = data.invMaxAmt.replace(/\,/g,"");
	}
}
