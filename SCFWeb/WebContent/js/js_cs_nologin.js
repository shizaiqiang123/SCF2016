$(function(){
	...
	//SYSBUSIUNIT系统栏位
	SCFUtils.SYSBUSIUNIT = $("#SYSBUSIUNIT").val().toUpperCase();
...
var clientData ={};
if (SCFUtils.getIframeWidow('iframepage')&&SCFUtils.getIframeWidow('iframepage').onCancelBtnClick&& $.isFunction(SCFUtils.getIframeWidow('iframepage').onCancelBtnClick)) {
	clientData = SCFUtils.getIframeWidow('iframepage').onCancelBtnClick();
}
var options = {
	url : SCFUtils.CANCEL,
	data : clientData||{reqLoginType:'noLogin'},
	async : false
};
SCFUtils.ajax(options);
...
function processFixPendingPage(data){
	if(SCFUtils.OPTSTATUS ==='RS'){
		...
		if(typeof pageOnResultLoad !== "undefined" && $.isFunction(pageOnResultLoad)){
			$.extend(options.data,{reqLoginType:'noLogin'});
	     	SCFUtils.ajax(options);
		}else{
 ...
 */
 SCFUtils.initBar = function(){
	...
	if(SCFUtils.CURRENTPAGE<SCFUtils.TOTALPAGE-1){
		...
		if("VW"!==SCFUtils.FUNCTYPE && "VIEW"!==SCFUtils.OPTSTATUS){
 ...
 SCFUtils.initNotice = function(){
 	if (typeof initNotice !== "undefined" && $.isFunction(initNotice)) {		
 		...
 		var blockAreaNotice = $('<div class="blockAreaNotice" id="blockAreaNotice"></div>');		
 		var noticeTxt = initNotice();
 		if(!SCFUtils.isEmpty(noticeTxt)){
 			$('<p class="fL blockAreaNoticeTitle red">温馨提示：</p>').appendTo(blockAreaNotice);
 			var blockAreaNoticeTxt = $('<p class="blockAreaNoticeTxt textblack" id="blockAreaNoticeTxt"></p>').appendTo(blockAreaNotice);
